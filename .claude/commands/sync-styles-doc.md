...
4. **Apply the hard rules:**
   ...
   - **Re-derive the table lists from the code.** Do not copy them from the previous version of the doc — the per-table classification in the source is the only source of truth.
   - **Re-derive the filter behaviour from the code.** Each filter's predicate-per-sync-style description must match what the code actually does.
   ...
   - **Match the existing doc's tone and layout.** Tight prose, short tables (no padded-column alignment), `---` separators between top-level sections, a "single useful invariant" callout near the end. Read the current `sync_styles.md` before writing to lock in the voice.
