...
| File | Change |
|------|--------|
| `server/service/src/invoice/inbound_shipment/update/mod.rs` | `received_datetime: Option<DateTime<Utc>>` field; error variants; `backdate_location_movements` handling; activity log entry for backdating |
| `server/service/src/invoice/inbound_shipment/update/validate.rs` | Validation: preference enabled, received status, only earlier (UTC datetime comparison), max backdating days |
| `server/service/src/invoice/inbound_shipment/update/generate.rs` | Updates `received_datetime` via `.naive_utc()`, moves shipped/delivered/created dates back, updates location movement `enter_datetime` values |
| `server/graphql/invoice/src/mutations/inbound_shipment/update.rs` | `received_datetime: Option<DateTime<Utc>>` on GraphQL input, error mapping |
...
| File | Change |
|------|--------|
| `client/packages/invoices/src/InboundShipment/DetailView/ReceivedDateInput.tsx` | Received date picker; sends `DateTime` via `.toISOString()`; maxDate set to day before current received to match server validation |
...
| `client/packages/common/src/authentication/api/operations.graphql` | `backdatingOfShipments` preference query |
