...
// deliberately small — one entry per locale — in the app's try/catch
// localStorage style (cf. appData.ts).
//
// Disabled entirely in dev: there LANG_VERSION is the constant 'dev', so the
// version check can never bust the cache, and a catalog edit would be masked by
// a stale entry until the TTL lapsed (7 days) or storage was cleared by hand.
// In dev the "load" is a local module import anyway (instant, HMR-invalidated
// on edit), so the cache buys nothing and only causes staleness. Prod keeps it
// — that's where a cache matters, for the real network fetch of server
// custom-translations.
