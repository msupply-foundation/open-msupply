...
- **[`hostBridge.ts`](./hostBridge.ts)** — the typed bridge (`window.electronNativeAPI`), wire-compatible with the current shell's preload (open-msupply `client/packages/electron/src/preload.ts`), so the existing Electron shell can host this page unmodified.
