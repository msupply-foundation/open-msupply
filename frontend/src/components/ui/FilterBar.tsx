 ...
 *
 * State model: the caller's filter object IS the state, in GraphQL-native shape. A
 * chip is shown iff its key is PRESENT on the filter (present-as-`null` = added but
 * empty). Adding writes `null`, removing deletes the key, editing goes through the
 * field's own control via setPartialFilter. Chip visibility therefore lives in the
 * (URL-backed) filter, so a restored state re-opens its chips — no local presentation
 * signal to seed. The page strips null/empty keys before querying (stripEmpty).
 */
