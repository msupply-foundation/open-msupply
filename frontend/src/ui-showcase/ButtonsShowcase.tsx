    ...
    <CardGrid minColumnWidth="32rem" maxColumnWidth="40rem">
      <DashboardCard title="Standard buttons — pre-composed for common actions">
        <Lead>
          ...
          <code>&lt;SaveButton&gt;</code>, <code>&lt;DialogSaveButton&gt;</code>
          , and <code>&lt;SaveAndNextButton&gt;</code>. Each fixes its own{' '}
          ...
          <strong>collapses to icon-only on phones</strong> — resize below 768px
          to see it; <code>DialogSaveButton</code>/
      ...
      <DashboardCard title="Primary — the main action">
        <Lead>
          ...
          for interaction (it needs the pointer coordinates). Tab to one for the
          focus ring. Variants are named semantically — never after a colour —
          the palette lives only in the tokens.
        </Lead>
        <Row>
      ...
      <DashboardCard title="Secondary — supporting actions">
        <Lead>
          <code>variant="secondary"</code>: an <strong>outlined</strong> button
          for actions that sit alongside a primary — Print, Export, Cancel,
          Edit. A hairline edge at rest; hover recolours the edge and label to
          the action tone. Use when there's a primary present; promote it to
          primary if it's the only action.
        </Lead>
        <Row>
      ...
      <DashboardCard title="Ghost — low-priority & inline">
        <Lead>
          ...
          primary's blue), with a faint tint on hover. Its home is inline table
          actions, overflow menus, and dialogs where space is tight — anywhere a
      ...
      <DashboardCard title="Danger — the high-caution action">
        <Lead>
          <code>variant="danger"</code>: a filled <strong>brand-orange</strong>{' '}
          tone for actions to be careful with — Delete, Remove, Void. It's a
          "stop and think" accent, <em>not</em> a hard error-red (it reuses the
          brand tone, matching the standard's own CSS). Never adjacent to a
          primary without a spacer. It's the{' '}
      ...
      <DashboardCard title="Sizes">
        <Lead>
          ...
          for dense tables and compact panels. At tablet widths (≤1023px) medium
          grows to the 48px touch target automatically — resize the window to
          see it; small is the deliberate exception.
        </Lead>
        <Row>
      ...
      <DashboardCard title="States">
        <Lead>
          ...
          <code>loading</code> swaps the icon for a spinner and blocks re-firing
          while staying at full opacity (busy, not disabled);{' '}
      ...
      <DashboardCard title="Icon button — the icon IS the button">
        <Lead>
          ...
          table toolbar controls, table row actions and panel actions — where a
          full labelled pill is too big. Plain <code>&lt;button&gt;</code> +
      ...
      <DashboardCard title="Split button — select-then-confirm (menuSelectsOnly)">
        <Lead>
          ...
          <code>menuSelectsOnly</code> +<code>onValueChange</code>; the default
          (pick = run) stays for export-style menus. Again in both{' '}
