    ...
    <Card
      ...
      lead={
        <>
          ...
          area inside a dialog (the create-stocktake include-all / filter block).
          Hand-rolled, pure CSS + tokens: no interaction or a11y contract to buy,
          just a tinted rounded container. Pairs with <code>FieldRow</code>.
        </>
      }
    >
      <div class={styles.formPreview}>
    ...
    <Card
      ...
      lead={
        <>
          ...
          radiogroup/radio ARIA for free; we only style the native control with{' '}
          <code>accent-color</code> (the brand dot) and lay the label — with an
          optional muted description — beside it. Options can be individually{' '}
          <code>disabled</code>, and <code>indentRem</code> lines a sub-group up
          under a sibling control. This is the create-stocktake type +
          include-all choice.
        </>
      }
    >
      <div class={styles.formPreview}>
        <RadioGroup
          ...
          options={[
            { value: 'full', label: 'Full stocktake', description: 'Counts every item in the store.' },
            { value: 'filtered', label: 'Filtered stocktake', description: 'Counts items matching the filters.' },
            { value: 'blank', label: 'Blank stocktake', description: 'Creates an empty stocktake.' },
          ]}
        />
        <div class={styles.radioSubgroup}>
          <RadioGroup
            ...
            options={[
              ...
              { value: 'all', label: 'All items', disabled: stocktakeType() === 'blank' },
            ]}
          />
        </div>
      </div>
