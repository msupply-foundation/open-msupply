...
- **Remember on success, not at choice** — the legacy screen records a server the moment it is chosen, so one that never connected can be auto-attempted forever. This page records only after the host's answer check passes — and records _before_ asking the host to navigate, so no host needs a grace delay for the page to persist under teardown.
