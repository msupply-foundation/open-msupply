...
// The same fragment backs both me and login, so both carry the same shape (spec, Guard 1).
...
// Spec (Unexpected logout / Inactivity): the re-login modal shows when a session
// exists and either a GraphQL call came back unauthenticated or the user was
// inactive too long. The user() guard keeps the startup me check (which also
// travels through graphqlFetch) from triggering the modal.
...
// Returns whether the check settled (authenticated or not — authUser reflects the
// outcome); false on a globally handled failure, so callers stay in their loading
// phase. Does not clear an existing user on an unauthenticated result — the re-login
// modal preserves the user's workflow (spec, Unexpected logout).
...
export type LoginResult =
  ...
  // Globally handled failure (unexpected-error modal): the consumer stays in its
  // loading phase.
  | { kind: 'pending' };
...
export const login = async (
  ...
  // Spec (Store Login): a user with no stores cannot log in. The backend enforces this
  // (NoSiteAccess); this is a defensive check only.
...
// Spec (Authentication Logic, Explicit logout): the backend clears the session cookie;
// clearing the user presents the login page. Unlike unexpected logout, no re-login
// modal. The local session ends regardless of the server response.
...
// Spec (Authentication Logic): inactivity forces the re-login modal; an active user
// with no recent GraphQL traffic triggers a token refresh call.
export const startActivityTracking = (): (() => void) => {
  ...
  const interval = window.setInterval(() => {
    ...
    if (msSinceLastGqlCall() > currentUser.tokenRefreshIntervalSeconds * 1000) {
      // Fire-and-forget: graphqlFetch never throws, and a failed ping needs no handling.
