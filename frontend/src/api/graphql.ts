...
// `NotAuthenticated("Missing auth token")`) while `message` is often just the generic
// category ("Unauthenticated"). Surface the details when present and distinct, so the
// unexpected-error modal shows what actually went wrong rather than the bare category.
const describeError = (e: GraphqlErrorItem): string => {
  const details = e.extensions?.details;
  if (typeof details === 'string' && details.length > 0 && details !== e.message) {
