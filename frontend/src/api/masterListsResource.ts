...
// App-wide master-lists cache: fetched once per store, shared/deduped,
// refetched on store change (createStoreScopedResource). Module-scope singleton
// so the dedup is real — every consumer reads this one instance. Read via
// `.noSuspense()` at the call site so a pending first load never trips a
// <Suspense> boundary (kdd/state-management: no remounts).
