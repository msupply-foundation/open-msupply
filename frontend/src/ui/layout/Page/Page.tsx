...
export interface PageProps {
  ...
  /**
   * The details-panel CONTENT — a stack of <SidePanelSection>s (or any markup). The PANEL ITSELF
   * (the docked frame, its header with the title, and the close button) is baked into the frame
   * here — a detail view supplies only what goes INSIDE, not the panel chrome. A DOCKED column at
   * the inline-end of the body: while `sidePanelOpen` it takes its width and PUSHES the body
   * (shrinks it), rather than floating over it. Omit on pages without a panel.
   */
  sidePanelContent?: JSX.Element
  /** The panel's title (header text + accessible name). */
  sidePanelTitle?: string
  /**
   * Whether the panel is shown. Set from a page-owned signal + a header toggle button to make the
   * panel openable (as in Open mSupply's details button). The page owns just the boolean; the
   * frame owns the panel chrome (header, close button, docking geometry).
   */
  sidePanelOpen?: boolean
  /** Called by the panel's own close button (top inline-end) — flip the page's open signal here. */
...
export const Page = (props: PageProps) => {
  ...
  return (
    // Top level is a ROW: the main column (header / body / footer) beside the details panel, so
    // the panel spans the WHOLE page height (alongside the header and footer too), pushing all of
    // them when it opens — not just the body band.
    <div class={styles.page}>
      ...
      <Show when={props.sidePanelContent && props.sidePanelOpen !== false}>
        <SidePanel label={props.sidePanelTitle} onClose={props.onSidePanelClose}>
