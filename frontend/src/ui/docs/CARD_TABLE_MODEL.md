...
| `meta.hideFromColumnSettings`   | `boolean`              | `false`       | Keep the column out of the **Columns popover** — it **stays on screen** (forced visible whatever the persisted config says), just not user show/hide/move/pin. For structural columns — the card identity, a row-actions column, a field a card can't do without.                      |
...
| `rowState`             | `(row) => 'verified' \| 'warning' \| 'disabled'`             | The row's **background** tint, mapped from the row's own facts — see [Row states & backgrounds](#row-states--backgrounds). Table view has all three; card view styles **`disabled`** only.   |
...
A row's colour comes from page-supplied functions, each a pure map from a row to a **semantic name** — never a colour (the CSS owns the palette, and colour never carries meaning alone: WCAG 2.2). Card view takes only two of them: a selected card gets a blue border + faint fill, `rowState`'s **`disabled`** mutes the card (below), and every other value — `rowTint`, `rowAccent`, and `rowState`'s selection-gated pair — is table-view only. `rowTone` / `cardTone` paint the card's identity title.
...
**In card view only `disabled` has a treatment** — a muted fill and secondary text, because a record that cannot be edited must read as one in either rendering (a line editor whose grid is cards at every width would otherwise show a barred row exactly like an editable one). The card's identity title keeps its own colour, tone included, exactly as a disabled row's tone survives the muting in table view. `verified` / `warning` are selection-gated tints of a row background, and a card has none, so they are stamped on the card row (`data-row-state`, the same attribute table view uses) but styled only for `disabled`.

The card's fill is its **own token**, `--table-card-surface-disabled` — _not_ the `--bg-toolbar` grey a disabled row takes. That grey is sized as a step off the **white row surface**, and a card sits on the **recessed** card list, so reusing it lands 5/255 from a normal card in light and 3/255 from the list floor in dark: invisible in one theme, and in the other the card box dissolves into its own list, leaving just the 1px border (Aneesh, PR [#1207](https://github.com/msupply-foundation/open-msupply-frontend/pull/1207)). A disabled card has to clear **both** of its neighbours — the card it isn't and the list it must not sink into — and the two themes have different room between them; the token's `tokens.css` note carries the measured deltas. Note the reach: because `rowState` is a row-level fact rather than a view choice, this lands on **every** card list that keys it off an editability gate — the requisitions, both returns, outbound, internal-orders and prescriptions list screens, plus the outbound and prescriptions line editors. That is the intent (those same rows are grey in table view), so a screen that greys most of its rows greys most of its cards.

### `rowTint` — the status background

`rowTint={(row) => 'unfinished' | 'success' | 'warning' | 'error' | undefined}` is the record-STATUS background channel, **always on** — unlike `rowState`'s selection-gated tints: `unfinished` (teal) for a row still awaiting work (an outbound line with nothing issued, an uncounted stocktake line), `error` (red) for a row the server refused (an R&R form line), `success` (green) and `warning` (amber) for a satisfied row and one needing attention. One tint per row; the page encodes its own precedence. Table view only — cards carry status via `cardTone` + badges.

**Spend it on one question.** A tint earns its place by answering what the table is _for_ — "what is left?", "what failed?" — and the live users each set exactly one value. Washing several statuses over the same rows spends the screen's one at-a-glance channel on facts the row's own badges already state, and it drowns out whichever of them the user is actually scanning for: outbound's line editor carried three (expired red > held amber > allocated green) and gave them all up for that reason (spec D111, 2026-08-21), keeping the words. Reach for a second tint value in one table only with a reason of that size. It shows through the `disabled` grey (the status is why the row is disabled; the muted text stays), and selection deepens the tint instead of switching to the selection blue. On **clickable** rows (`onRowClick` set — the detail tables) hover deepens the tint further; non-clickable grids (the line editors) have no hover response. Never colour alone: the tint restates a fact a cell states in words. A prop function reading a store field (the outbound editor's `numberOfPacks`) re-evaluates on in-place edits — the tint flips live, with none of the TanStack accessor-caching trouble.
...
- **Structural columns opt out of the Columns popover** with `hideFromColumnSettings` — the identity title, a row-actions column, and either face of a two-faces split. Setting it also **forces the column visible**: dropping the popover row alone would leave a `false` already stored for that id (in the user layer or the store-wide global blob) hiding the column with nothing able to reach it — the popover filters the id out of both its eye toggles and its Show all / Hide all, so only the Settings popover's _Show all columns_ or a full reset could bring it back, and nothing on screen would say a field had gone missing. `resolveColumnVisibility` ([columnTypes.ts](../elements/table/columnTypes.ts)) applies the force for every table, and the next visibility write heals the stale entry. NB TanStack's own `enableHiding: false` would **not** do this — it gates the handler, not the state. (The selection checkbox is chrome gated by `enableSelection`, not a column, so it's never in the popover to begin with.) **When EVERY column is structural the toolbar drops the Columns control** rather than opening a panel holding nothing but _Show all_ / _Hide all_ — `listedColumnIds` ([ColumnSettings.tsx](../elements/table/ColumnSettings.tsx)) answers "what would this panel list" once, for the panel and for the trigger's `<Show>`. That is a whole grid of fields, not an oversight: the demographics grid's seven columns each carry an in-place field or the year's growth-rate input, and a save writes all of them whatever is on screen, so hiding one withdraws the only way to set a value that still gets written. The Settings popover beside it (density, full-screen, _Reset table to default_) is unaffected.
