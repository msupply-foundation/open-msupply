...
**In card view only `disabled` has a treatment** — the card takes the same muted fill and secondary text, because a record that cannot be edited must read as one in either rendering (a line editor whose grid is cards at every width would otherwise show a barred row exactly like an editable one). The card's identity title keeps its own colour, tone included, exactly as a disabled row's tone survives the muting in table view. `verified` / `warning` are selection-gated tints of a row background, and a card has none, so they are stamped on the card row (`data-row-state`, the same attribute table view uses) but styled only for `disabled`.
