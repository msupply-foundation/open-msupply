...
| `surface`     | `--bg-white` `--surface-raised` `--table-card-surface` `--bg-drawer` `--bg-menu` `--bg-toolbar` `--bg-row` `--bg-group-light` `--bg-group-main` `--bg-group-dark` `--bg-input` `--bg-disabled` `--header-bg` `--drawer-selected-bg` `--drawer-hover-bg` `--bg-login` — plus three **derivation-only** members no role names: `--table-card-surface-disabled`, `--bg-group-main`, `--bg-group-dark` (each rides `page`; naming the first as a settable member would let an author paint a disabled card the same colour as the list it sits on) |
