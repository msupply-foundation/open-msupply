...
- [ ] Every single-line input is `size="small"` — `width="compact"` for a short value (%, date, code), no width at all for free text (name, reference, note — filling is the default); comments are `TextArea` (rule 2).
