...
// `onInteractOutside` for an interaction it judges to be outside the popup — on
// a pointerdown, but ALSO on a focus change. When the popup is portal-mounted
// INTO a native <dialog> (so it isn't inert / behind the top layer — see
// utils/portalMount), clicking an option is mis-reported as outside via that
// FOCUS path:
//
...
// dismisses the listbox between mousedown and pointerup — the click never lands
// on the option and selection silently fails (keyboard still works; it fires no
// focus-to-dialog bounce).
//
