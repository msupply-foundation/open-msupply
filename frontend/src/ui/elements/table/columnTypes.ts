...
import type { ColumnDef, IdentifiedColumnDef, RowData } from '@tanstack/solid-table';
import type { LocaleKey } from '../../../intl';

// The DataTable's COLUMN MODEL — pure types + pure functions, no reactivity or JSX. Split out of
// DataTable.tsx so a page (or tableHelpers) can import a `Column` type without pulling in the 60KB
// component module, and so the column shape lives in one place. DataTable.tsx re-exports these so
// existing consumers keep importing from './DataTable'.

// An untyped display convention → TanStack's `meta` bag (kdd/table-state: meta for flags
// that need no table-specific type). Augmented here so it's typed everywhere columnDef.meta
// is read (the cell helpers in tableHelpers.ts set align; the renderers read it). A convention
// that needs the K generic goes on Column<T,K> instead.
declare module '@tanstack/solid-table' {
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  interface ColumnMeta<TData extends RowData, TValue> {
    /** Text alignment for the cell + header — a display-only convention TanStack has no
     *  concept of; read by the renderers and applied via a data-align attribute. */
...
export const membershipCardGroups = (m: Membership): string[] => (Array.isArray(m) ? m : []);
...
//     in DataTable, and kdd/edit-line-card-table.
...
// view — hence the icon (shown on the tab + card group row) and the translated label.
