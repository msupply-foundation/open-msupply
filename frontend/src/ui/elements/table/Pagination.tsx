...
export const Pagination = (props: PaginationProps) => {
  ...
  return (
    // Rendered even at zero rows ("Showing 0-0 of 0", pager disabled on its one
    // page) — the footer is part of the list's stable chrome, as in the current
    // app; an appearing/disappearing bar would shift the layout on every filter.
    <nav class={styles.pagination} aria-label={t('pagination.label')}>
      ...
      <span class={styles.summary} aria-live="polite">
        {t('pagination.showing')} <strong class={styles.summaryNumber}>{from()}-{to()}</strong>{' '}
        {t('pagination.of')} <strong class={styles.summaryNumber}>{props.total}</strong>
      </span>
