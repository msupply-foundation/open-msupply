...
// Row-grouping aggregation (kdd/table-state) — the grouped-parent value each column shows. Pure
// TanStack AggregationFns, split out of DataTable.tsx so tableHelpers can use them without the
// component (and without the old DataTable ⇄ tableHelpers import cycle).
//
// When rows are GROUPED (see DataTable rowGroup) a parent row stands in for its leaves, and each
// column decides what its parent cell shows via TanStack's own `aggregationFn` (set directly on the
// column — we don't wrap it). TanStack ships 'sum' etc. by name; here are the extra custom ones a
// caller (or the cell helpers) can hand to `aggregationFn`.

// MULTIPLE is ONE typed, global constant so the "[multiple]" placeholder is spelled once; the
// renderer shows it literally, matching Open mSupply.
