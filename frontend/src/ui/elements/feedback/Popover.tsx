...
export interface PopoverProps {
  ...
  /** Open on hover (and focus) as well as click — for content bubbles whose trigger IS the
      content (a status row), where hover reads more naturally than a click. Click still works. */
  openOnHover?: boolean
  class?: string
...
export const Popover = (props: PopoverProps) => {
  ...
  // Hover-open: show on pointer-enter / focus of the trigger, hide once the pointer has left BOTH
  // the trigger and the panel (a small delay lets the pointer travel across the gap). Click still
  // toggles (native popovertarget). Keyboard focus opens it too, so it's not hover-only (a11y).
