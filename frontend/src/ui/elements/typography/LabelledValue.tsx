 ...
 * region and in detail/side panels (ui-standards § tables, card layout: labels
 * muted above values). Stacked (label on top), so a row of these WRAPS
 * intrinsically (CLAUDE.md #7) rather than forcing a fixed grid. Composes
 * <Text> for type only (bodySmall label / body value); the colours are set
 * here (label secondary, value body) — Text never carries colour.
 *
 * Deliberately hand-rolled, pure CSS — no interaction/a11y contract to buy. A
 * plain block (not a <dl>): key-value-as-description-list is optional for
 * read-only display, and a bare block wraps and reuses more freely.
 */
