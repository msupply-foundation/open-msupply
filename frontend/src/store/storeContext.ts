...
// The store the loaded context belongs to — keyed on the REQUEST, not derived from
// the response: storePreferences.id is the store id only when the store has a
...
// never confirm and the store guard would refetch forever.
...
// on it without a component. Set only when the store guard has resolved and authorised
// the store from the URL and its context fetch succeeded — so it is URL-driven in
// effect (kdd/url-structure), and never reports a store whose lookups aren't yet valid
