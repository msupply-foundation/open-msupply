...
import { InternalOrderMasterLists } from './masterList.generated';
import styles from './MasterListPickerModal.module.css';

// The master-list picker (spec/internal-orders S7): a modal listing the store's
// own non-program master lists, one row per name, filtered by a search field.
...
const PickerContent = (props: MasterListPickerModalProps): JSX.Element => {
  ...
  onMount(() => {
    void (async () => {
      const result = await graphqlFetch(InternalOrderMasterLists, {
