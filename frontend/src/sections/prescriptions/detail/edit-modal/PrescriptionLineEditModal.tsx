...
export interface PrescriptionLineEditModalProps {
  ...
  programId?: string;
  /**
   * The prescribed quantity is READ-ONLY on this prescription — it came from a
   * prescription request and is the prescriber's figure, not the dispenser's
   * (spec/prescriptions/rules.md § prescribed quantity). Set when the
   * dispensation was generated by a hand-over. The field still SHOWS: it is
   * the target the dispenser is allocating against, and hiding it would take
   * away the one number they most need.
   */
  lockPrescribedQuantity?: boolean;
  onClose: () => void;
...
const Body = (props: PrescriptionLineEditModalProps) => {
  ...
  // ...but not when the figure is the prescriber's and this screen may only
  // read it: a locked field is not an entry point, so Issue — the field the
  // dispenser is actually here to fill — takes the focus instead.
  const focusPrescribedFirst = () =>
    prefs().editPrescribedQuantity && props.lockPrescribedQuantity !== true;
      ...
      <FormSection title={t('label.item')} headingLevel="h3" heading="group">
        <ItemSearch
          ...
          onSelect={item => {
            ...
            if (focusPrescribedFirst()) prescribedQuantityFocus.focus();
          }}
        />
      </FormSection>
          ...
          <FormRow class={styles.quantityRow}>
            <Show when={prefs().editPrescribedQuantity}>
              <NumberField
                ...
                min={0}
                decimalLimit={0}
                disabled={props.lockPrescribedQuantity}
