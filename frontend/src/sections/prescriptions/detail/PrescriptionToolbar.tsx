...
//
// On a dispensation GENERATED from a prescription request, Patient and
// Clinician are read-only however editable the record itself is: they are the
// prescriber's entries, copied across at the hand-over from a request that is
// itself locked, so changing them here would silently disagree with it (issue
// #513; spec/prescriptions § fields the prescriber owns). A surface
// restriction only — the write stays open, as with the prescribed quantity.
...
> = props => {
  ...
  // Generated by a hand-over — see the note above. It only ADDS to the
  // record's own editability gate, never relaxes it.
