...
import { DeletePrescriptionOrderLine } from './edit-modal/orderLineEdit.generated';
import {
  savePrescriptionOrder,
  type UpdateInput,
} from './prescriptionOrderUpdate';
import { PrescriptionOrderToolbar } from './PrescriptionOrderToolbar';
import {
  PrescriptionOrderSidePanel,
  type PrescriptionOrderEditFields,
} from './PrescriptionOrderSidePanel';
import { PrescriptionOrderStatusFooter } from './PrescriptionOrderStatusFooter';
import { OrderLineEditModal } from './edit-modal/OrderLineEditModal';
import { EditPatientModal } from '../../patients';

// The prescription-order detail (spec/prescription-orders/ui-surface.md S3):
...
const PrescriptionOrderDetailView: Component = () => {
  ...
  // The order — header, side panel, AND lines in one read (a small line set;
  ...
  const [data, { mutate, refetch }] = createResource(
    ...
    async variables => {
      const result = await graphqlFetch(PrescriptionOrderDetail, variables, {
        mapSuccessToError: d =>
          d.prescriptionOrder.__typename === 'RecordNotFound'
            ? d.prescriptionOrder.description
            : undefined,
      });
      if (result.kind !== 'success') return undefined;
      return result.data.prescriptionOrder.__typename === 'PrescriptionOrderNode'
        ? result.data.prescriptionOrder
        : undefined;
    }
  );
  ...
  const info = (): PrescriptionOrderFieldsFragment | undefined => gated(data);

  const status = () => asOrderStatus(info()?.status ?? 'DISPENSED');
  ...
  const tableConfig = createTableConfig({
    tableId: 'prescription-order-detail',
  });

  const saveField = async (input: Omit<UpdateInput, 'id'>) => {
    const node = info();
    if (!node) return;
    const outcome = await savePrescriptionOrder(params.storeId, {
      id: node.id,
      ...input,
    });
  ...
  // order identity.
  const edit = createDebouncedEdit<PrescriptionOrderEditFields>({
    id: () => info()?.id ?? '',
    initial: () => ({
  ...
  const runDeleteLines = async () => {
    for (const id of selectedIds()) {
      const result = await graphqlFetch(DeletePrescriptionOrderLine, {
        storeId: params.storeId,
        id,
      });
  ...
  const crumbs = (node?: PrescriptionOrderFieldsFragment) => [
    {
      label: t('prescriptions'),
      to: `/${params.storeId}/dispensary/prescription-order`,
    },
    { label: node ? `${node.prescriptionOrderNumber}` : '…' },
  ];
          ...
          <Page
            ...
            sidePanelContent={
              <PrescriptionOrderSidePanel
                ...
                onDeleted={() =>
                  navigate(
                    `/${params.storeId}/dispensary/prescription-order`,
                    { replace: true }
                  )
                }
              />
...
export default PrescriptionOrderDetailView;
