...
import { savePrescriptionOrder } from './prescriptionOrderUpdate';
import type { PrescriptionOrderFieldsFragment } from './prescriptionOrderDetail.generated';

// The detail status footer (spec/prescription-orders/ui-surface.md S3 §
...
export interface PrescriptionOrderStatusFooterProps {
  storeId: string;
  node: PrescriptionOrderFieldsFragment;
  /** The hand-over saved — merge the returned node in place. */
  onSaved: (node: PrescriptionOrderFieldsFragment) => void;
}

export const PrescriptionOrderStatusFooter: Component<
  PrescriptionOrderStatusFooterProps
> = props => {
  ...
  const run = async () => {
    ...
    setRejection(undefined);
    const outcome = await savePrescriptionOrder(
      props.storeId,
