...
| **S2 detail**       | ✅ partial        | Stats band (`StatsPanel`/`Statistic` in the header `Toolbar`, above the tabs) + URL-driven deep-linkable tabs. Built: General/Store/Master-lists/Ledger(`ItemLedgerPanel`)/Ancillary(`ItemAncillaryPanel`)/Custom-fields/Log(`ActivityLogPanel`) + not-found→list. Flagged: Variants (placeholder).                                                              |
