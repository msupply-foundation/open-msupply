...
describe('the calculation (rules § the calculation)', () => {
  it('AC-C1 — current population is the baseline scaled by the share, to a whole person', () => {
    expect(currentPopulation(1000, 33.33)).toBe(333);
  });

  it('AC-C2 — each year compounds on the previous year’s ROUNDED figure', () => {
...
describe('the grid order (rules § the grid)', () => {
  it('AC-G1 — pins the general population row first and keeps the read’s order for the rest', () => {
    const fetched = [
  ...
  it('AC-G5 — no stored record reads as zero rates and no record id', () => {
  ...
  it('AC-G4 — stored rates reach the headers as stored, out-of-range or not', () => {
    // AC-P4: the server validates no range, so a record another client wrote
...
describe('new indicator (rules § editing the draft, § indicators)', () => {
  it('AC-N1 — a blank editable row: no name, 0 % share, unsaved', () => {
...
describe('what a save sends (rules § saving the draft; contract § indicators)', () => {
  ...
  it('AC-S2 — every existing row is written in full with the baseline as its base population and recomputed projections', () => {
  ...
  it('AC-D4 — the general population row is written back under the screen’s own label', () => {
  ...
  it('AC-N2 — a new row is created with the base year it was born with and its computed figures', () => {
  ...
  it('AC-N3 — a blank name is OMITTED from a create, never sent as ""', () => {
  ...
  it('AC-P2 — the stored growth-rate record is updated with all five rates and the base year', () => {
  ...
  it('AC-P1 — with no record stored, the rates are CREATED under a fresh client id', () => {
