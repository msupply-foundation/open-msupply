...
const Body: Component<InboundShipmentLineEditModalProps> = props => {
  ...
  const columns = (): Column<DraftBatch, never, GroupKey>[] => [
    ...
    // receiver needs it in front of them alongside expiry — not one click down.
    {
      ...
      cardGroup: 'batch',
      // 12.5–30rem, the configured-lookup pair (ui-standards § Field Widths by
      // Content Type). A location renders as `code — name` ("fr — Central
      ...
      meta: { cardWidth: { min: 12.5, max: 30, weight: 1.2 } },
      cell: info => {
        const b = info.row.original;
        return (
          <LocationVolumeSelect
      ...
      ? [
          {
            ...
            cardGroup: 'pricing',
            meta: { cardWidth: { min: 12.5, max: 30, weight: 1 } },
            cell: info => {
              const b = info.row.original;
              return (
                <NameSearch
    ...
    // own primary zone to about this count for the same reason.
    {
      ...
      // 12, then 4+8.
      meta: { cardWidth: { min: 12.5, max: 30, weight: 1.4 } },
      cell: info => {
        const b = info.row.original;
        return (
          <NameSearch
