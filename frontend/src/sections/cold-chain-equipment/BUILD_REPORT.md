...
- **The status-history timeline has no connector rail and no avatar glyph.** `ui-surface.md` S2.3 describes both, transcribed from the reference screen, but neither is a role the [registry](../../../spec/ui-standards/components.md) names — so building them would be inline improvisation, which the detail-views standard rules out ("When a vertical needs a control the registry doesn't yet name for a field, that is a new registry row, not an inline improvisation"). The entries are stacked detail cards instead, which carries the separation the rail was doing. Either the registry gains a timeline row or the surface drops the rail; it needs a decision, and it is the vertical's second candidate spec refinement.
