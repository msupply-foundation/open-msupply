...
export interface StatusChangeActionProps {
  ...
  /** ≥1 line — advancing an empty return is blocked (OMS-REG-DIST-07.27). */
  hasLines: boolean;
...
// acting (D39); hold blocks exactly this advance (OMS-REG-DIST-07.7), releasing clears it.
//
...
export const StatusChangeAction: Component<StatusChangeActionProps> = props => {
  ...
  const openConfirm = (status: string) => {
    // No lines / on hold: explain rather than dead-end (OMS-REG-DIST-07.27/.7's UI
  ...
  return (
    <>
      ...
      {/* Blocked advance (no lines / on hold): an info-only dialog explaining
          why the advance can't run yet (OMS-REG-DIST-07.27/.7). */}
      <Dialog
