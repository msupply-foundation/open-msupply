...
// Whether the return has actually put stock on the shelf. Stock is introduced
// on the FIRST transition into RECEIVED or VERIFIED and at no earlier step
// (rules § what receiving does), so a transfer return sitting at PICKED or
// SHIPPED holds none.
const hasIntroducedStock = (status: string): boolean =>
  status === 'RECEIVED' || status === 'VERIFIED';

// Whether DELETING the return would take that stock back out, which is what the
// delete confirmation warns about (rules § deletion rules).
//
// Holding stock is not enough. A VERIFIED return is finalised, so its delete is
// refused outright (server invoice/customer_return/delete/validate.rs →
...
// only picks the copy. At PICKED or SHIPPED there is simply no stock yet —
// telling someone their shipped return "has already been received" would be
...
// the caller narrows the flow by the invoice-status-options preference
// (@/domain/invoice filterByStatusPreference; rules § preference gates) —
