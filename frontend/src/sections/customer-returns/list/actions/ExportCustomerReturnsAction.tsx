...
// covers every row matching the active filters, across all pages). If a store
// somehow exceeds the cap the export is truncated, so that's logged rather than
// silently shipped.
const EXPORT_PAGE_SIZE = 10000;

export const ExportCustomerReturnsAction: Component<
  ExportCustomerReturnsActionProps
> = props => {
  ...
  const buildCsv = async (): Promise<string | null> => {
    const result = await graphqlFetch(CustomerReturns, {
    ...
    if (totalCount > nodes.length) {
      console.warn(
        `customer-returns export: truncated to ${nodes.length} of ${totalCount} matching rows (cap ${EXPORT_PAGE_SIZE}).`
      );
    }
    return nodes.length ? customerReturnsToCsv(nodes) : null;
  };
  ...
  const run = async (format: string): Promise<void> => {
    ...
    try {
      ...
      const listName = t('filename.customer-returns');
      if (format === 'excel') {
