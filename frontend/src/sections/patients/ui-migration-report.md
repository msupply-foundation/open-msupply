...
**Status: MIGRATED (one finding deliberately deferred).** Nine of ten findings applied, two spec edits and one doc edit made, `pnpm check` and `pnpm test` green, and the reactivity pass over the diff came back clean. **F5 (the create wizard's step rail) is deferred by the operator's call** — it is being done as its own step so it can be reviewed on its own, and spec edit **S2** waits with it so spec and code stay in agreement. **The visual pass is yours to complete** — the list is at the end, and it matters more than usual here because the custom-fields change is app-wide.
...
| **F5**    | **Deferred** — the wizard step rail stays hand-rolled for now, by the operator's call (its own review step)                                                                           | 1/6/9/11 |
...
| S2 `list/CreatePatientModal.tsx`                                     | ⏸️ F5      | ✅            | ✅ F2    | ✅       | ✅             | ⏸️ F5     | ✅           | ⚠️ N2   | ⏸️ F5  | ✅ F10        | ⏸️ F5   |
...
### F5 — The create wizard hand-rolls its step rail with inline styles (dims 1, 6, 9, 11) · **DEFERRED**
...
**Deferred by the operator (2026-07-31)** so it can be built and reviewed as its own step. Nothing else in this
migration depends on it: the block is left byte-for-byte as it was, and S2 waits with it so the spec still matches
the code. The two inline `style` blocks in `CreatePatientModal` are therefore the only ones left in the vertical.
...
| **S2** _(deferred with F5)_ | `spec/patients/ui-surface.md` header + § S2 (+ [`README.md`](../../../spec/patients/README.md) known gaps)  | the **wizard stepper** is no longer a registry gap — name the existing determinate-progress-list role; the schema-driven document form stays the one gap                                                                       | the role exists and is built (F5) — **not made**, so spec and code stay in agreement while the rail is hand-rolled   |
...
3. **F5 — deferred.** The wizard step rail is its own step, to be reviewed on its own; spec edit S2 waits with it.
...
- **The wizard step rail** (`CreatePatientModal`) — **not uncovered after all**: the registry's determinate progress list (`ProgressList`) fills it, as F5 sets out. It stays hand-rolled only because the swap is deferred to its own step. This is the one place in the vertical where an inline `style` remains.
...
5. **Create wizard** `New patient` — the results table's widths; the step rail is **deliberately unchanged** (F5's own step).
