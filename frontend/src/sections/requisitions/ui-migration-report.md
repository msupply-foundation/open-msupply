...
| Detail — line editor (edit + read-only open) | internal-orders line editor | read-only figures as text lines up with the input column edge; Save & next enabled on read-only and walking without saving; captions rhythm |
