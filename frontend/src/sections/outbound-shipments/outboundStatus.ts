...
// The customer-return entry point (spec/outbound-shipments acceptance
// OMS-REG-DIST-04.21): "Return selected lines" opens the customer-return flow
// only from SHIPPED, DELIVERED or VERIFIED — the shipment has left the store,
// so its issued lines can come back. At any other status (RECEIVED included,
// per the UI matrix) the button instead shows the explanatory notice. The
// button is always visible; this only decides open-flow vs notice.
