// The client-side FEFO distribution behind the line editor's Issue field
// (spec/outbound-shipments S4): fill usable batches oldest-expiry-first in
// WHOLE packs — never split a pack, round the last take UP within availability
// (over-allocation < 1 pack, reported) — and report the shortfall (the
// placeholder quantity while NEW) plus whether any usable-looking stock was
// skipped as barred. Pure: the modal owns the draft store; this owns the
// arithmetic so AC-A1/A3/A4's client face is testable in isolation.
...
export type Distribution = {
  ...
  /** Units requested but not covered by usable stock (→ placeholder). */
