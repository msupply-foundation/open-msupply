...
- **Campaign/program field** (S2/S3) is not built — needs a combined campaign-or-program lookup + a campaigns query; the live backend's `StockLineFilterInput` also lacks `campaignId` (drift from the pinned schema).
