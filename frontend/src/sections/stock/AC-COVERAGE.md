...
Colocated unit tests: `stockCalc.test.ts`, `stockApi.test.ts`, `stockLocations.test.ts`, `list/stockToCsv.test.ts` (31 tests).

| AC                                         | Tier           | Where                                                                                                 |
| ------------------------------------------ | -------------- | ----------------------------------------------------------------------------------------------------- |
| AC-L1 only packs-on-hand                   | e2e            | `hasPacksInStore: true` always sent (`StockList.variables`/`ExportStockAction`)                       |
...
| AC-L5 grouped-by-item (deferred)           | —              | deferred this iteration (spec/stock D63); the list is the flat stock-line list only                   |
...
| AC-L9 card view                            | ui             | `DataTable` card view — per-column `headerPosition`/`cardGroup` in `StockList.columns`; `CARD_GROUPS` |
...
| AC-P2 doses context                        | ui             | dose suffix on the list (`unitsText`), the detail quantity fields (`qtyValue`), and the adjust tiles  |
