...
// memory). The filter object is a plain flat shape (NOT a GraphQL filter — these lines never go
// back to the server as a query), consumed by FilterBar the same way: a key present = an active
// chip, absent = not added. `search` is the always-on item search (name OR code, like OMS);
// `errorIds` is the special errors-only filter the error dialog switches on (only meaningful when
// there ARE error lines).

export interface StocktakeLineFilter {
  /** Always-on free text — matches item name OR code (case-insensitive substring), like OMS. */
  search?: string;
  batch?: string;
  ...
  item?: string;
  /** When present, keep ONLY lines whose id is in this set (the errors-only filter). */
  errorIds?: string[];
}
...
// Does a line pass every active filter? An absent/empty key is inactive (doesn't constrain).
export const lineMatchesFilter = (
  ...
  if (filter.search && filter.search.trim()) {
    ...
    if (!includesCI(line.itemName, q) && !includesCI(line.item.code, q)) return false;
  }
  ...
  if (filter.location && filter.location.trim()) {
    ...
    if (!includesCI(line.location?.code, q) && !includesCI(line.location?.name, q)) return false;
  }
  if (filter.expiryBefore) {
    // A line with no expiry never satisfies an "expiring before" bound.
    if (!line.expiryDate || line.expiryDate >= filter.expiryBefore) return false;
  }
  if (filter.errorIds) {
