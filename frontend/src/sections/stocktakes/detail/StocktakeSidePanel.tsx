...
// "Entered by" / "Created", then the editable Counted by / Verified by / Comment. The three
// editable fields read/write the shared stocktake edit buffer (owned by the view, one across the
// whole entity) — each keystroke writes the store and saves debounced, coalescing with each other
// AND with the toolbar's description into a single updateStocktake (kdd/state-management). The
// buffer decouples the inputs from the async mutation (typing is smooth, the fields never jump when
// a save returns) and re-seeds only when the stocktake identity changes.

export interface StocktakeSidePanelProps {
  node: StocktakeInfoFragment;
  disabled: boolean;
  /** The shared stocktake edit buffer — this panel reads/writes countedBy / verifiedBy / comment. */
