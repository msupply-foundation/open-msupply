...
| **.19** / **.20**          | Phase list with progress (count where countable); idle keeps completed phases                   | `syncStatus.test.ts` › _phase list with progress_ (`toSyncOverview` — the per-phase done/total data); the D100 placement (count beneath the in-flight phase only; a completed phase's count on marker hover/focus) is the shared `ui/sync/ProgressList`'s rendering    |
