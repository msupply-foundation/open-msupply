...
export type DashboardGates = {
  /** External-inbound panel — procurement capability on (AC-R2). */
  externalInboundPanel: boolean;
  /** Emergency (new) customer-requisition stat — program module on (AC-T3). */
  emergencyStat: boolean;
  /** Out-of-stock (recently used) stat — consumption look-back set (AC-S3). */
  outOfStockRecentlyUsedStat: boolean;
  /** At-risk stat — low-stock-alert threshold > 0 (AC-S6). */
  atRiskStat: boolean;
  /** Overstocked stat — over-stock-alert threshold > 0 (AC-S7). */
  overstockedStat: boolean;
  /** Expiring-between-thresholds stat — at least one expiry threshold > 0 (AC-E4). */
...
// pauses the fetch rather than sending threshold-less variables (AC-S8).
