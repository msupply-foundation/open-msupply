...
- **Customer-requisition stat links** are unfiltered where the spec asks for New / New+emergency filters. That is a registered placeholder pending the customer-requisition vertical ([statLinks.ts:136](statLinks.ts#L136)), already recorded in `BUILD_REPORT.md`. Behaviour, not composition — out of migration scope.
