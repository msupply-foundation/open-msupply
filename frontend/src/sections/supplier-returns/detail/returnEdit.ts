...
// SupplierReturnDetailView and passed whole to the children, so a burst of
// edits coalesces into a single updateSupplierReturn (kdd/state-management).
// Empty strings stand in for null fields; the header patch cannot CLEAR a field
// back to absent — only overwrite (contract § header rules) — so '' is sent
// as-is.
