...
// The returns-list bulk delete (spec/supplier-returns § deletion) — the
// outbound DeleteShipmentsAction shape: the whole batch is refused when ANY
// selected return is shown read-only (Shipped and beyond — deletable only while
// NEW or PICKED) — a UI pre-check with a blocking notice instead of the
// confirmation, no server call; per-row enforcement remains server-side. There
// is NO batch mutation for supplier returns, so a confirmed batch runs one
// deleteSupplierReturn per id; the pre-check means those should all succeed —
// any server rejection (a status changed under a stale list) still lands in the
// error phase as a backstop. A clean sweep closes silently (closure is the
// confirmation — ui-standards controls.md § dialogs). The hand-back to the list
// (clear selection + re-query) is DEFERRED to the dialog's close: clearing the
// selection collapses the selection-gated footer this dialog lives in, so
// calling it mid-flow unmounts the dialog before the error phase can show.
...
> = props => {
  ...
  const onClick = () => {
    ...
    if (
      props
        .selectedRows()
        .some(row => row.status !== 'NEW' && row.status !== 'PICKED')
    ) {
      setBlockedOpen(true);
      return;
    }
