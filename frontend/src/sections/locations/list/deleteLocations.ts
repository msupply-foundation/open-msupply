...
// (contract.md § deletion), each succeeding or failing on its own (AC-D3).
...
export type DeleteOutcome =
  /** AC-D1 — the location was removed. */
  | { kind: 'deleted'; id: string }
  /**
   * AC-D2 — the typed LocationInUse rejection, carrying the referencing
   * stock-line / invoice-line counts that block the delete.
   */
  | { kind: 'inUse'; id: string; stockLines: number; invoiceLines: number }
  /**
   * AC-D5 — an untyped failure: a location with stock-movement history fails
...
export type DeleteSummary = {
  deletedCount: number;
  /** The in-use report rows (AC-D2/AC-D3) — blocked, with their references. */
  inUse: Extract<DeleteOutcome, { kind: 'inUse' }>[];
  /** Untyped failures (AC-D5) — reported by count only. */
  failedCount: number;
};

/**
 * AC-D3 — fold per-location outcomes: every unused location is deleted and
