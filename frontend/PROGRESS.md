...
| **e2e**      | `e2e/specs/<vertical>-regression.spec.ts` exists (the stem may differ from the folder by a trailing `s` — `stocktakes` is driven by `stocktake-regression.spec.ts`) | `/author-deterministic-tests` |
...
The table is derived from the tree, and `pnpm check` re-derives it: `scripts/check-progress.mjs`
fails when any cell disagrees with what is present, when a vertical in `spec/` has no row,
or when a row names a vertical that is gone. So a vertical that finishes a step ticks its
own cell **in the PR that delivers that step**, the same way it adds its `spec/README.md`
index line — and a PR that forgets is told.

A cross-cutting folder that is not a vertical belongs in [Not listed](#not-listed) above;
the check reads that section, so naming it there is what excludes it.
