...
mod query {
    ...
    async fn temperature_breach_service_pagination() {
        let (_, _, connection_manager, _) =
            setup_all("test_temperature_breach_service_pagination", MockDataInserts::all()).await;
        ...
        assert_eq!(
            service.get_temperature_breachs(
                &context,
    ...
    async fn temperature_breach_service_single_record() {
        let (_, _, connection_manager, _) =
            setup_all("test_temperature_breach_single_record", MockDataInserts::all()).await;
        ...
        assert_eq!(result.temperature_breach_row.id, "temperature_breach_acknowledged");
    ...
    async fn temperature_breach_service_filter() {
        ...
        let result = service
            .get_temperature_breachs(
                &context,
                None,
                Some(TemperatureBreachFilter::new().id(EqualFilter::equal_to("temperature_breach_1"))),
                None,
            )
            .unwrap();

        assert_eq!(result.count, 1);
        assert_eq!(result.rows[0].temperature_breach_row.id, "temperature_breach_1");

        let result = service
            .get_temperature_breachs(
                &context,
                None,
                Some(TemperatureBreachFilter::new().id(EqualFilter::equal_any(vec![
        ...
        assert_eq!(result.rows[0].temperature_breach_row.id, "temperature_breach_1");
        assert_eq!(result.rows[1].temperature_breach_row.id, "temperature_breach_acknowledged");
    }
    ...
    async fn temperature_breach_service_sort() {
        ...
        let mut temperature_breachs = mock_data["base"].temperature_breachs.clone();
        temperature_breachs.sort_by(|a, b| a.start_timestamp.cmp(&b.start_timestamp));
        ...
        let result_timestamps: Vec<NaiveDateTime> = result
            ...
            .collect();
        let sorted_timestamps: Vec<NaiveDateTime> = temperature_breachs.into_iter().map(|temperature_breach| temperature_breach.start_timestamp).collect();
        ...
        let mut temperature_breachs = mock_data["base"].temperature_breachs.clone();
        temperature_breachs.sort_by(|a, b| b.end_timestamp.cmp(&a.end_timestamp));
        ...
        let result_timestamps: Vec<NaiveDateTime> = result
            ...
            .collect();
        let sorted_timestamps: Vec<NaiveDateTime> = temperature_breachs.into_iter().map(|temperature_breach| temperature_breach.end_timestamp).collect();
