...
pub fn get_sensor(ctx: &ServiceContext, id: String) -> Result<Sensor, SingleRecordError> {
    ...
    let mut result =
        repository.query_by_filter(SensorFilter::new().id(EqualFilter::equal_to(id.to_owned())))?;
...
pub fn get_sensor_logs_filter_for_breach(
    ...
    let filter = TemperatureLogFilter::new()
        .sensor(SensorFilter::new().id(EqualFilter::equal_to(breach.sensor_id.to_owned())))
