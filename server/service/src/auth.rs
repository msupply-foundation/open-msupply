...
impl AuthServiceTrait for AuthService {
    fn validate(
        ...
        let mut permission_filter =
            UserPermissionFilter::new().user_id(EqualFilter::equal_to(user_id.to_owned()));
        if let Some(store_id) = &resource_request.store_id {
            permission_filter = permission_filter.store_id(EqualFilter::equal_to(store_id.to_owned()));
        }
