...
fn generate(
    ...
    let customer_name_id = StoreRepository::new(connection)
        .query_by_filter(StoreFilter::new().id(EqualFilter::equal_to_string(&ctx.store_id)))?
        .pop()
...
fn generate_program_indicator_values(
    ...
    let customer_name_ids: Vec<String> = NameRepository::new(connection)
        .query(
            ...
            Some(
                NameFilter::new()
                    .supplying_store_id(EqualFilter::equal_to_string(store_id))
                    .is_customer(true)
                    .is_store(true),
            ),
            None,
...
mod test_insert {
    ...
    async fn insert_program_request_requisition_success_service() {
        ...
        let requisition_lines = RequisitionLineRepository::new(&connection)
            .query_by_filter(
                RequisitionLineFilter::new().requisition_id(EqualFilter::equal_to_string(&new_row.id)),
            )
            .unwrap();
