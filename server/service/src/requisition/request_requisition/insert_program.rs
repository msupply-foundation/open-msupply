...
fn generate(
    ...
    let program_item_ids: Vec<String> = MasterListLineRepository::new(connection)
        .query_by_filter(
            MasterListLineFilter::new().master_list_id(EqualFilter::equal_to(master_list_id.to_owned())),
            None,
        )?
        .into_iter()
    ...
    let program_indicators = program_indicators(
        ...
        Pagination::all(),
        None,
        Some(ProgramIndicatorFilter::new().program_id(EqualFilter::equal_to(program.id.to_owned()))),
    )?;

    let customer_name_id = StoreRepository::new(connection)
        .query_by_filter(StoreFilter::new().id(EqualFilter::equal_to(ctx.store_id.to_owned())))?
        .pop()
...
fn generate_program_indicator_values(
    ...
    let customer_name_ids: Vec<String> = NameRepository::new(connection)
        .query(
            ...
            Some(
                NameFilter::new()
                    .supplying_store_id(EqualFilter::equal_to(store_id.to_owned()))
                    .is_customer(true)
                    .is_store(true),
            ),
            None,
...
mod test_insert {
    ...
    async fn insert_program_request_requisition_success_service() {
        ...
        let requisition_lines = RequisitionLineRepository::new(&connection)
            .query_by_filter(
                RequisitionLineFilter::new().requisition_id(EqualFilter::equal_to(new_row.id.to_owned())),
            )
            .unwrap();
