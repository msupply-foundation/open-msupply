...
pub fn check_requisition_exists(
    ...
    Ok(RequisitionRepository::new(connection)
        .query_by_filter(RequisitionFilter::new().id(EqualFilter::equal_to_string(id)))?
        .pop())
}
...
pub fn get_lines_for_requisition(
    ...
    RequisitionLineRepository::new(connection).query_by_filter(
        RequisitionLineFilter::new().requisition_id(EqualFilter::equal_to_string(requisition_id)),
    )
}
...
pub fn check_emergency_order_within_max_items_limit(
    ...
    let program_settings_ids = ProgramRequisitionSettingsRepository::new(connection)
        .query(Some(ProgramRequisitionSettingsFilter::new().program(
            ProgramFilter::new().id(EqualFilter::equal_to_string(program_id)),
        )))?
        .iter()
    ...
    match order_type {
        Some(order_type) => {
            let mut filter = RequisitionFilter::new()
                .program_id(EqualFilter::equal_to_string(input.program_id))
                .order_type(EqualFilter::equal_to_string(&order_type.name))
                .period_id(EqualFilter::equal_to_string(input.period_id))
                .store_id(EqualFilter::equal_to_string(input.store_id))
                .r#type(input.requisition_type.equal_to());

            if let Some(other_party_id) = input.other_party_id {
                filter = filter.name_id(EqualFilter::equal_to_string(other_party_id));
            };
