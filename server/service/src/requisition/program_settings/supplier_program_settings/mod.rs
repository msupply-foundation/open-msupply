...
#[derive(Debug, PartialEq)]
pub struct SupplierProgramSettings {
...
pub fn get_supplier_program_requisition_settings(
    ...
    let Some(prepared) = prepare_supplier_program_settings(ctx, store_id)? else {
        return Ok(Vec::new());
    };

    // Map program_settings, order_types, periods and requisitions_in_periods to ProgramSettings
    Ok(map_supplier_program_settings(prepared))
}
...
mod test {
    ...
    async fn get_supplier_program_requisition_settings() {
        ...
        let mut result = service_provider
            .requisition_service
            .get_supplier_program_requisition_settings(&service_context, &mock_store_a().id)
            .unwrap();
        result.sort_by(|a, b| {
