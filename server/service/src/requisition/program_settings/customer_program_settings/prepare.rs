...
pub(super) fn prepare_program_requisition_settings_by_customer(
    ...
    let filter = ProgramRequisitionSettingsFilter::new()
        .master_list(
            MasterListFilter::new().exists_for_name_id(EqualFilter::equal_to(customer_name_id.to_owned())),
        )
        .name_tag(NameTagFilter::new().name_id(EqualFilter::equal_to(customer_name_id.to_owned())));
    ...
    let filter = RequisitionsInPeriodFilter::new()
        .other_party_id(EqualFilter::equal_to(customer_name_id.to_owned()))
