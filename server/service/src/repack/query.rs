...
pub fn get_repack(ctx: &ServiceContext, invoice_id: &str) -> Result<Repack, RepositoryError> {
    ...
    let invoice = InvoiceRepository::new(connection)
        .query_by_filter(
            InvoiceFilter::new()
                .id(EqualFilter::equal_to(invoice_id.to_owned()))
...
mod test {
    ...
    async fn query_repacks() {
        ...
        let invoice_a = InvoiceRepository::new(&context.connection)
            .query_by_filter(
                InvoiceFilter::new()
                    .id(EqualFilter::equal_to(repack.invoice_row.id.to_owned()))
                    .store_id(EqualFilter::equal_to(context.store_id.to_owned()))
                    .r#type(InvoiceType::Repack.equal_to()),
            )
            .unwrap()
            .pop()
        ...
        let invoice_a_lines = InvoiceLineRepository::new(&context.connection)
            .query_by_filter(
                InvoiceLineFilter::new()
                    .invoice_id(EqualFilter::equal_to(invoice_a.invoice_row.id.to_owned())),
            )
            .unwrap();
        ...
        let invoice_b = InvoiceRepository::new(&context.connection)
            .query_by_filter(
                InvoiceFilter::new()
                    .id(EqualFilter::equal_to(invoice.id.to_owned()))
                    .store_id(EqualFilter::equal_to(context.store_id.to_owned()))
                    .r#type(InvoiceType::Repack.equal_to()),
            )
            .unwrap()
        ...
        let invoice_b_lines = InvoiceLineRepository::new(&context.connection)
            .query_by_filter(
                InvoiceLineFilter::new()
                    .invoice_id(EqualFilter::equal_to(invoice_b.invoice_row.id.to_owned())),
            )
            .unwrap();
