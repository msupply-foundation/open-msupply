...
pub fn get_repack(ctx: &ServiceContext, invoice_id: &str) -> Result<Repack, RepositoryError> {
    ...
    let invoice = InvoiceRepository::new(connection)
        .query_by_filter(
            InvoiceFilter::new()
                .id(EqualFilter::equal_to_string(invoice_id))
...
mod test {
    ...
    async fn query_repacks() {
        ...
        let invoice_a = InvoiceRepository::new(&context.connection)
            .query_by_filter(
                InvoiceFilter::new()
                    .id(EqualFilter::equal_to_string(&repack.invoice_row.id))
                    .store_id(EqualFilter::equal_to_string(&context.store_id))
                    .r#type(InvoiceType::Repack.equal_to()),
            )
            .unwrap()
            .pop()
        ...
        let invoice_a_lines = InvoiceLineRepository::new(&context.connection)
            .query_by_filter(
                InvoiceLineFilter::new()
                    .invoice_id(EqualFilter::equal_to_string(&invoice_a.invoice_row.id)),
            )
            .unwrap();
        ...
        let invoice_b = InvoiceRepository::new(&context.connection)
            .query_by_filter(
                InvoiceFilter::new()
                    .id(EqualFilter::equal_to_string(&invoice.id))
                    .store_id(EqualFilter::equal_to_string(&context.store_id))
                    .r#type(InvoiceType::Repack.equal_to()),
            )
            .unwrap()
        ...
        let invoice_b_lines = InvoiceLineRepository::new(&context.connection)
            .query_by_filter(
                InvoiceLineFilter::new()
                    .invoice_id(EqualFilter::equal_to_string(&invoice_b.invoice_row.id)),
            )
            .unwrap();
