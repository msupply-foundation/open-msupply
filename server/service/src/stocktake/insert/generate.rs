...
fn generate_stocktake_lines(
    ...
    if let Some(master_list_id) = master_list_id {
        stock_line_filter = stock_line_filter
            .master_list(MasterListFilter::new().id(EqualFilter::equal_to(master_list_id.to_owned())))
    }

    if let Some(location_id) = location_id {
        stock_line_filter = stock_line_filter
            .location(LocationFilter::new().id(EqualFilter::equal_to(location_id.to_owned())))
    }

    if let Some(vvm_status_id) = vvm_status_id {
        stock_line_filter = stock_line_filter.vvm_status_id(EqualFilter::equal_to(vvm_status_id.to_owned()));
    }
    ...
    let item_ids: Vec<String> = MasterListLineRepository::new(connection)
        .query_by_filter(
            MasterListLineFilter::new()
                .master_list_id(EqualFilter::equal_to(master_list_id.to_owned()))
