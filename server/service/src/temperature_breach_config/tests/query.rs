...
mod query {
    use repository::{
        ...
        temperature_breach_config::{TemperatureBreachConfigFilter, TemperatureBreachConfigSortField},
        test_db::setup_all,
    };
    ...
    async fn temperature_breach_config_service_pagination() {
        let (_, _, connection_manager, _) =
            setup_all("test_temperature_breach_config_service_pagination", MockDataInserts::all()).await;
    ...
    async fn temperature_breach_config_service_single_record() {
        let (_, _, connection_manager, _) =
            setup_all("test_temperature_breach_config_single_record", MockDataInserts::all()).await;
        ...
        let result = service
            .get_temperature_breach_config(&context, "temperature_breach_config_is_active".to_owned())
            .unwrap();

        assert_eq!(result.temperature_breach_config_row.id, "temperature_breach_config_is_active");
    ...
    async fn temperature_breach_config_service_filter() {
        let (_, _, connection_manager, _) =
            setup_all("test_temperature_breach_config_filter", MockDataInserts::all()).await;
        ...
        let result = service
            .get_temperature_breach_configs(
                &context,
                None,
                Some(TemperatureBreachConfigFilter::new().id(EqualFilter::equal_to("temperature_breach_config_1"))),
                None,
            )
            .unwrap();

        assert_eq!(result.count, 1);
        assert_eq!(result.rows[0].temperature_breach_config_row.id, "temperature_breach_config_1");

        let result = service
            .get_temperature_breach_configs(
                &context,
                None,
                Some(TemperatureBreachConfigFilter::new().id(EqualFilter::equal_any(vec![
        ...
        assert_eq!(result.rows[0].temperature_breach_config_row.id, "temperature_breach_config_1");
        assert_eq!(result.rows[1].temperature_breach_config_row.id, "temperature_breach_config_is_active");
    }

    #[actix_rt::test]
    async fn temperature_breach_config_service_sort() {
        let (mock_data, _, connection_manager, _) =
            setup_all("test_temperature_breach_config_sort", MockDataInserts::all()).await;
