use super::{
    query::get_temperature_breach_config,
    validate::check_temperature_breach_config_exists,
};
use crate::{service_provider::ServiceContext, SingleRecordError};
use repository::{
    temperature_breach_config::TemperatureBreachConfig, RepositoryError, TemperatureBreachConfigRow, TemperatureBreachConfigRowRepository, StorageConnection,
};
...
pub fn update_temperature_breach_config(
    ...
    let temperature_breach_config = ctx
        ...
        .transaction_sync(|connection| {
            ...
            let updated_temperature_breach_config_row = generate(input, temperature_breach_config_row);
            TemperatureBreachConfigRowRepository::new(&connection).upsert_one(&updated_temperature_breach_config_row)?;

            get_temperature_breach_config(ctx, updated_temperature_breach_config_row.id).map_err(UpdateTemperatureBreachConfigError::from)
        })
...
pub fn validate(
    ...
    let temperature_breach_config_row = match check_temperature_breach_config_exists(&input.id, connection)? {
        Some(temperature_breach_config_row) => temperature_breach_config_row,
        None => return Err(UpdateTemperatureBreachConfigError::TemperatureBreachConfigDoesNotExist),
    };

    if temperature_breach_config_row.store_id != Some(store_id.to_string()) {
        return Err(UpdateTemperatureBreachConfigError::TemperatureBreachConfigDoesNotBelongToCurrentStore);
    }
...
pub fn generate(
    ...
    temperature_breach_config_row.description = description.unwrap_or(temperature_breach_config_row.description);
    temperature_breach_config_row.is_active = is_active.unwrap_or(temperature_breach_config_row.is_active);
