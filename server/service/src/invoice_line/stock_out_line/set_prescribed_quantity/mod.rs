        ...
        .transaction_sync(|connection| {
            ...
            let existing_lines = InvoiceLineRepository::new(connection).query_by_filter(
                InvoiceLineFilter::new()
                    ...
                    .invoice_id(EqualFilter::equal_to(input.invoice_id.to_owned())),
            )?;
...
mod test {
    ...
    async fn set_prescribed_quantity_no_stock_line() {
        ...
        let filter = InvoiceLineFilter::new().invoice_id(EqualFilter::equal_to(mock_prescription_unallocated_invoice_line().invoice_id.to_owned()));
