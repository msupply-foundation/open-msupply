...
impl<'a> UserAccountService<'a> {
            ...
            .transaction_sync(|con| {
                ...
                let permissions_to_delete = UserPermissionRepository::new(con).query_by_filter(
                    UserPermissionFilter::new()
                        .user_id(EqualFilter::equal_to_string(&user.id))
                        .has_context(false),
                )?;
    ...
    pub fn find_user_active_on_this_site(
        ...
        repo.query_one(
            UserFilter::new()
                .id(EqualFilter::equal_to_string(user_id))
...
mod user_account_test {
    ...
    async fn test_user_upsert() {
        ...
        let user = user_repo
            .query_by_filter(UserFilter::new().id(EqualFilter::equal_to_string(&mock_user_account_a().id)))
            .unwrap()
            .pop()
            .unwrap();
        ...
        let permissions = user_permission_repo
            .query_by_filter(
                UserPermissionFilter::new()
                    .user_id(EqualFilter::equal_to_string(&mock_user_account_a().id)),
            )
            .unwrap();
        ...
        let user = user_repo
            .query_by_filter(UserFilter::new().id(EqualFilter::equal_to_string(&mock_user_account_b().id)))
            .unwrap()
            .pop()
            .unwrap();
        ...
        let permissions = user_permission_repo
            .query_by_filter(
                UserPermissionFilter::new()
                    .user_id(EqualFilter::equal_to_string(&mock_user_account_b().id)),
            )
            .unwrap();
