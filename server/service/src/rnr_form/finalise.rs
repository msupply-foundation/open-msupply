...
fn validate(
    ...
    let lines = RnRFormLineRepository::new(connection)
        .query_by_filter(RnRFormLineFilter::new().rnr_form_id(EqualFilter::equal_to(input.id.to_owned())))?;
...
fn generate(
    ...
    let rnr_form_lines = RnRFormLineRepository::new(&ctx.connection).query_by_filter(
        RnRFormLineFilter::new().rnr_form_id(EqualFilter::equal_to(rnr_form_id.to_owned())),
    )?;
