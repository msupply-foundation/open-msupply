...
mod query {
    use repository::mock::{
        mock_name_tag_1, mock_name_tag_2, mock_period_2_d, mock_period_schedule_2, MockData,
    };
    ...
    async fn get_schedules_with_next_periods_by_program() {
        ...
        // sorted in descending order, so latest period should be first
        assert_eq!(periods[0].period_row.id, mock_period_2_d().id);
