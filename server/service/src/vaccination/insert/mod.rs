    ...
    for (index, dose) in all_course_doses.iter().enumerate() {
        ...
        let existing_vaccination = VaccinationRepository::new(connection).query_one(
            VaccinationFilter::new()
                .vaccine_course_dose_id(EqualFilter::equal_to(dose.vaccine_course_dose_row.id.to_owned()))
                .program_enrolment_id(EqualFilter::equal_to(program_enrolment.id.to_owned())),
        )?;
