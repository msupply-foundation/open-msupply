...
mod tests {
    ...
    /// Regression guard for the v2.16 -> v2.17 breaking change, where the foreign-key fields of
    /// several translators flipped between the bare `*_id` name and the `*_link_id` wire name
    /// without a version bump, silently corrupting sync. The snapshot must pin the canonical
    /// `*_link_id` wire keys so any reversion is caught as a contract diff.
    #[test]
    fn link_id_wire_keys_are_pinned() {
        let schema = sync_schema();
        let record_schemas = &schema["translators"]["record_schemas"];

        let wire_keys = |translator: &str| -> Vec<String> {
            record_schemas[translator]["properties"]
                .as_object()
                .unwrap_or_else(|| panic!("no `properties` in record schema for '{}'", translator))
                .keys()
                .cloned()
                .collect()
        };

        // (translator, must contain *_link_id, must NOT contain the bare *_id v2.16-era key)
        let cases = [
            ("rnr_form", "name_link_id", "name_id"),
            ("rnr_form_line", "item_link_id", "item_id"),
            ("item_variant", "item_link_id", "item_id"),
            ("vaccination", "patient_link_id", "patient_id"),
            ("vaccine_course_item", "item_link_id", "item_id"),
        ];
        for (translator, present, absent) in cases {
            let keys = wire_keys(translator);
            assert!(
                keys.contains(&present.to_string()),
                "'{}' wire schema must pin '{}' (got {:?})",
                translator,
                present,
                keys
            );
            assert!(
                ...
                "'{}' wire schema must NOT expose the bare '{}' key (that is the v2.16-era format \
                 the v2.17 rename broke)",
                translator,
                absent
            );
        }

        // item_variant also renames its manufacturer FK; vaccination renames item + facility FKs.
        assert!(
            wire_keys("item_variant").contains(&"manufacturer_link_id".to_string()),
            "item_variant must pin 'manufacturer_link_id'"
        );
        for key in ["item_link_id", "facility_name_link_id"] {
            assert!(
                wire_keys("vaccination").contains(&key.to_string()),
                "vaccination must pin '{}'",
                key
            );
        }
    }
