...
impl SyncTranslation for IndicatorValue {
    ...
    fn try_translate_from_upsert_sync_record(
        ...
        let customer_name_link_id = StoreRepository::new(connection)
            .query_one(StoreFilter::new().id(EqualFilter::equal_to_string(&customer_store_id)))?
            .ok_or(anyhow::anyhow!(
    ...
    fn try_translate_to_upsert_sync_record(
        ...
        let Some(indicator_value) = IndicatorValueRepository::new(connection)
            .query_by_filter(
                IndicatorValueFilter::new().id(EqualFilter::equal_to_string(&changelog.record_id)),
            )?
            .pop()
        else {
        ...
        let customer_store_id = StoreRepository::new(connection)
            .query_one(StoreFilter::new().name_id(EqualFilter::equal_to_string(&customer_name_link_id)))?
            .ok_or(anyhow::anyhow!(
