...
/// `property_option_v2` row under the matching `legacy_name_category_*` property
/// (seeded by `central_mapping_properties`); the name stores the chosen leaf id
/// under its `categoryN` key (see `translations/name.rs`).
///
...
/// belong to. The three category1 levels share `legacy_name_category_1`; the
/// flat dimensions map 1:1. Must match `central_mapping_properties`.
fn property_id_for_table(table_name: &str) -> Option<&'static str> {
    match table_name {
        "name_category1" | "name_category1_level1" | "name_category1_level2" => {
            Some("legacy_name_category_1")
        }
        "name_category2" => Some("legacy_name_category_2"),
        "name_category3" => Some("legacy_name_category_3"),
        "name_category4" => Some("legacy_name_category_4"),
        "name_category5" => Some("legacy_name_category_5"),
        "name_category6" => Some("legacy_name_category_6"),
        _ => None,
    }
}
...
mod tests {
    ...
    async fn name_category_emits_property_option_on_central() {
        ...
        assert!(debug.contains("legacy_name_category_1"), "{debug}");
        assert!(
