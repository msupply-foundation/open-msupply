...
fn sync_upsert_document(
    ...
    let Some(registry) = DocumentRegistryRepository::new(con)
        .query_by_filter(
            DocumentRegistryFilter::new().document_type(EqualFilter::equal_to_string(&document.r#type)),
        )?
        .pop()
    else {
...
fn update_program_enrolment(
    ...
    let program_row = ProgramRepository::new(con)
        .query_one(ProgramFilter::new().context_id(EqualFilter::equal_to_string(&document.context_id)))?
...
fn update_encounter(con: &StorageConnection, document: &Document) -> Result<(), RepositoryError> {
    ...
    let existing_encounter = EncounterRepository::new(con)
        .query_by_filter(
            EncounterFilter::new().document_name(EqualFilter::equal_to_string(&document.name)),
        )?
        .pop();
