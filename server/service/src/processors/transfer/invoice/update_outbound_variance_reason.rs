...
const DESCRIPTION: &str = "Copy shipment variance reason from inbound to outbound";

pub(crate) struct UpdateOutboundVarianceReasonProcessor;

impl InvoiceTransferProcessor for UpdateOutboundVarianceReasonProcessor {
    fn get_description(&self) -> String {
        DESCRIPTION.to_string()
    }

    /// Reason will be copied onto outbound invoice lines when all below conditions are met:
    /// 1. Source invoice name_id is for a store active on this site (driver enforces this)
    /// 2. Source invoice is an Inbound Shipment
    ...
    fn try_process_record(
        ...
        let updated = set_variance_reason_on_outbound_lines(
            &ctx.connection,
        ...
        let result = format!(
            "({}) outbound lines variance reason updated ({}) from inbound ({})",
...
fn set_variance_reason_on_outbound_lines(
    ...
    for inbound_line in inbound_lines {
        ...
        if outbound_line.reason_option_id == inbound_line.invoice_line_row.reason_option_id {
            continue;
        }
