...
impl InvoiceTransferTester {
        ...
        let response_requisition = RequisitionRepository::new(connection)
            .query_one(
                RequisitionFilter::new()
                    .linked_requisition_id(EqualFilter::equal_to(self.request_requisition.id.to_owned())),
            )
            .unwrap();
            ...
            InvoiceLineRepository::new(connection)
                .count(Some(
                    InvoiceLineFilter::new()
                        .invoice_id(EqualFilter::equal_to(inbound_shipment.id.to_owned()))
                ))
                .unwrap(),
            3
        );
    ...
    pub(crate) fn check_inbound_shipment_deleted(&mut self, connection: &StorageConnection) {
        ...
        assert_eq!(
            InvoiceLineRepository::new(connection)
                .count(Some(
                    InvoiceLineFilter::new().invoice_id(EqualFilter::equal_to(inbound_shipment_id.to_owned()))
                ))
                .unwrap(),
            0
        );
    ...
    pub(crate) fn check_customer_return_created(&mut self, connection: &StorageConnection) {
        ...
        assert_eq!(
            InvoiceLineRepository::new(connection)
                .count(Some(
                    InvoiceLineFilter::new().invoice_id(EqualFilter::equal_to(customer_return.id.to_owned()))
                ))
                .unwrap(),
            1
        );
    ...
    pub(crate) fn check_customer_return_deleted(&mut self, connection: &StorageConnection) {
        ...
        assert_eq!(
            InvoiceLineRepository::new(connection)
                .count(Some(
                    InvoiceLineFilter::new().invoice_id(EqualFilter::equal_to(customer_return_id.to_owned()))
                ))
                .unwrap(),
            0
        );
...
fn check_line(
    ...
    let inbound_line = InvoiceLineRepository::new(connection)
        .query_one(
            InvoiceLineFilter::new()
                .invoice_id(EqualFilter::equal_to(inbound_id.to_owned()))
                .item_id(EqualFilter::equal_to(outbound_line.item_link_id.to_owned())),
        )
        .unwrap();
