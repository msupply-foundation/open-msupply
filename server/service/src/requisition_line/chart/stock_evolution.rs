...
use util::{constants::APPROX_NUMBER_OF_DAYS_IN_A_MONTH_IS_30, date_with_days_offset};
...
fn calculate_projected_stock_evolution(
    ...
    let average_daily_consumption = average_monthly_consumption / APPROX_NUMBER_OF_DAYS_IN_A_MONTH_IS_30;
    let mut result = Vec::new();
    let mut running_stock_on_hand = reference_stock_on_hand as f64;
    ...
    fn test_calculate_projected() {
        assert_eq!(
            calculate_projected_stock_evolution(
                30,
                25.5 * APPROX_NUMBER_OF_DAYS_IN_A_MONTH_IS_30, // amc
                100,
                NaiveDate::from_ymd_opt(2021, 1, 5).unwrap(),
                vec![
                    NaiveDate::from_ymd_opt(2021, 1, 3).unwrap(),
