...
pub struct InboundReturnLine {
    ...
    pub stock_line: StockLine, // TODO: this might need to be invoice line??
}
...
pub fn generate_inbound_return_lines(
    ...
    let return_lines: Vec<InboundReturnLine> = stock_lines
        .iter()
        .map(|stock_line| InboundReturnLine {
            id: uuid(),
...
mod test {
    ...
    #[actix_rt::test]
    async fn generate_inbound_return_lines_stock_line_ids() {
        let (_, _, connection_manager, _) = setup_all(
            "generate_inbound_return_lines_stock_line_ids",
            MockDataInserts::all(),
        )
        .await;
