...
mod test {
    ...
    async fn add_new_stock_line_success() {
        ...
        let mut invoice_lines = InvoiceLineRepository::new(&connection)
            .query_by_filter(
                InvoiceLineFilter::new().invoice_id(EqualFilter::equal_to(invoice_row.id.to_owned())),
            )
            .unwrap();
        ...
        let invoice_lines = InvoiceLineRepository::new(&connection)
            .query_by_filter(
                InvoiceLineFilter::new().invoice_id(EqualFilter::equal_to(invoice.invoice_row.id.to_owned())),
            )
            .unwrap()
            .pop()
            .unwrap();
        ...
        let vvm_status_log = VVMStatusLogRepository::new(&connection)
            .query_by_filter(
                VVMStatusLogFilter::new()
                    .stock_line_id(EqualFilter::equal_to(stock_line_id.to_owned()))
                    .invoice_line_id(EqualFilter::equal_to(invoice_lines.invoice_line_row.id.to_owned())),
            )
            .unwrap()
            .pop();
