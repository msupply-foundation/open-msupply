...
pub fn generate(
    ...
    let old_invoice_lines = InvoiceLineRepository::new(connection).query_by_filter(
        InvoiceLineFilter::new().invoice_id(EqualFilter::equal_to(existing_invoice.id.to_owned())),
    )?;
    ...
    let new_activity_log = ActivityLogRepository::new(connection)
        .query_by_filter(
            ActivityLogFilter::new().record_id(EqualFilter::equal_to(old_invoice.id.to_owned())),
        )?
        .iter()
