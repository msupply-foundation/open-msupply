...
pub fn generate(
    ...
    let old_invoice_lines = InvoiceLineRepository::new(connection).query_by_filter(
        InvoiceLineFilter::new().invoice_id(EqualFilter::equal_to_string(&existing_invoice.id)),
    )?;
    ...
    let new_activity_log = ActivityLogRepository::new(connection)
        .query_by_filter(
            ActivityLogFilter::new().record_id(EqualFilter::equal_to_string(&old_invoice.id)),
        )?
        .iter()
