...
pub fn validate(
    ...
    let purchase_order = PurchaseOrderRepository::new(connection)
        .query_by_filter(PurchaseOrderFilter::new().id(EqualFilter::equal_to_string(&input.id)))?
        .pop()
    ...
    let purchase_order_lines = PurchaseOrderLineRepository::new(connection).query_by_filter(
        PurchaseOrderLineFilter::new()
            .purchase_order_id(EqualFilter::equal_to_string(&purchase_order.purchase_order_row.id)),
    )?;
