...
impl EncounterNode {
    ...
    pub async fn active_program_events(
        ...
        let mut program_filter = filter
            ...
            .unwrap_or_default()
            .patient_id(EqualFilter::equal_to_string(&self.patient_row().id))
            .document_type(EqualFilter::equal_to_string(&self.encounter_row().document_type));
        if filter.and_then(|f| f.is_current_encounter).unwrap_or(false) {
            program_filter = program_filter
                .document_name(EqualFilter::equal_to_string(&self.encounter_row().document_name))
        };
        let list_result = ctx
            .service_provider()
