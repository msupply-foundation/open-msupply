...
impl RequisitionNode {
    ...
    pub async fn created_datetime(&self) -> DateTime<Utc> {
        DateTime::from_naive_utc_and_offset(self.row().created_datetime.clone(), Utc)
    }
    ...
    pub async fn sent_datetime(&self) -> Option<DateTime<Utc>> {
        ...
        sent_datetime.map(|v| DateTime::from_naive_utc_and_offset(v, Utc))
    }

    pub async fn finalised_datetime(&self) -> Option<DateTime<Utc>> {
        let finalised_datetime = self.row().finalised_datetime.clone();
        finalised_datetime.map(|v| DateTime::from_naive_utc_and_offset(v, Utc))
    }
