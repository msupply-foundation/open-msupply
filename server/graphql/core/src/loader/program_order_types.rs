...
impl Loader<OrderTypesByProgramIdInput> for OrderTypesByProgramIdLoader {
    ...
    async fn load(
        ...
        tokio::task::spawn_blocking(
            move || -> Result<
                HashMap<OrderTypesByProgramIdInput, Vec<ProgramAndOrderType>>,
                async_graphql::Error,
            > {
                ...
                let settings_to_store: HashMap<String, ProgramSettingsAndOrderTypes> = store_ids
                    .iter()
                    .filter_map(|store_id| {
                        get_program_settings_and_order_types_for_store(
                            &service_context,
                            store_id,
                        )
                        .ok()
                ...
                let mut result =
                    HashMap::<OrderTypesByProgramIdInput, Vec<ProgramAndOrderType>>::new();
                    ...
                    for item_id in item_ids {
                        ...
                        for program_id in &program_ids_for_item {
                            if let Some(data) = settings_to_store.get(&store_id) {
                                let program_ids_order_types: HashMap<
                                    String,
                                    Vec<ProgramAndOrderType>,
                                > = data
                                    .settings
                                    .iter()
                                    .filter(|s| {
                                        s.program_settings_row.program_id == *program_id
                                    })
                                    .map(|s| {
                                        ...
                                        (
                                            s.program_settings_row.program_id.clone(),
                                            order_types,
                                        )
                                    })
                                    .collect();
                                if let Some(mut p) =
                                    program_ids_order_types.get(program_id).cloned()
                                {
