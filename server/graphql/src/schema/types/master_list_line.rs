...
use crate::{loader::ItemLoader, standard_graphql_error::StandardGraphqlError, ContextExt};
...
impl MasterListLineNode {
    ...
    pub async fn item(&self, ctx: &Context<'_>) -> Result<ItemNode, StandardGraphqlError> {
        let loader = ctx.get_loader::<DataLoader<ItemLoader>>();
        let item_option = loader
            .load_one(self.master_list_line.item_id.clone())
            .await?;
        let item = item_option.ok_or(StandardGraphqlError::InternalError(format!(
