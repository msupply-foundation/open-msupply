...
// TODO warning msg
// Remove app data
// conneciton location in preferences ?
// Issue with TODOS
// Issue for selecting config location with env or startup arguments ?
// Add metadata to backup
// Better formatted db errors
...
fn get_backup_dir(input_dir: String) -> Result<Dirs, BackupError> {
    let base_dir =
        PathBuf::from_str(&input_dir).map_err(|_| BackupError::InvalidPath(input_dir.clone()))?;
...
fn copy_files(settings: &Settings, backup_file_dir: &PathBuf) -> Result<(), BackupError> {
    let restore_file_dir = get_file_dir(settings)?;
