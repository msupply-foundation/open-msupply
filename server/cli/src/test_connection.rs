...
async fn main() {
    ...
    let test_task = tokio::spawn(preform_tests(gui_tx, args));
...
async fn preform_tests(gui_tx: mpsc::Sender<GuiState>, args: Args) {
    let gui = args.gui;
...
impl Test for ConfigTest {
    async fn run(&self, test_data: &mut TestData) -> Result<String> {
        ...
        Ok("Successfully loaded config".to_string())
    }
}
...
impl Test for PingTest {
    async fn run(&self, test_data: &mut TestData) -> Result<String> {
        ...
        let url = Url::parse(&format!("http://{}/", address)).map_err(|err| {
            anyhow!(
...
impl Test for LoginTest {
    async fn run(&self, test_data: &mut TestData) -> Result<String> {
        ...
        let address = format!("http://{}/", config.server.address());
        info!("Testing login at url: {}", address);
        info!("    Username: {}", username);
        info!("    Password: {}", password);

        let url = Url::parse(&address).map_err(|err| anyhow!("Failed to parse URL: {:?}", err))?;
