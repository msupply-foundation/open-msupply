...
pub fn get_sync_actors() -> (SyncSenderActor, SyncReceiverActor) {
    ...
    (SyncSenderActor { sender }, SyncReceiverActor { receiver })
}
...
impl SyncReceiverActor {
    pub async fn listen(&mut self, delay_duration: Duration) {
        while let Some(()) = self.receiver.recv().await {
            info!("Received sync message. Simulating sync...");
            time::delay_for(delay_duration).await;
            info!("Finished simulating sync!");
        }
        unreachable!(
