...
| 21 immunisation programs (`e2e_prog_imm_*`) + their `context` rows                                                                                                | **nothing creates a program** — not either front end, not the API (programs are master lists flagged for immunisation, synced from legacy mSupply) — so Programs › Immunizations has rows only if the dump carries them. Three named for the case-insensitive sort (`e2e alpha lower` sorts FIRST only when the order ignores case), eighteen fillers so the list pages at every page size both front ends offer, `master_list_id` null so no requisition or R&R picker (`existsForStoreId`) ever sees them |
| `E2E Retired Programme`, an immunisation program with `deleted_datetime` set                                                                                      | the list's exclusion of an inactive program (`OMS-REG-IMM-01.1`) needs one to be absent                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `E2EImmunisationEnrolment` / `E2EImmunisationEncounter` document registries on `E2E EPI Routine`'s context, with the permissive `e2e_schema_imm_open` form schema | the in-use guards need a **vaccination** recorded against a dose, and a vaccination needs an encounter on a patient enrolled in the course's program — the suite creates all three through the API at run time, which needs these registries (the reference data has none for an immunisation program)                                                                                                                                                                                                      |
| `E2E Vaccine A` (visible to GRY) and `E2E Vaccine Hidden` (on a master list GRY is not joined to)                                                                 | the vaccine-item pick is scoped to the logged-in store's visible vaccines (`.32`): a second visible vaccine beside `DPT Vaccine` for the remove-one round trip, and one the store must NOT be offered                                                                                                                                                                                                                                                                                                       |
| `DocumentMutate` + `DocumentQuery` for Admin on GRY **in the `E2E EPI Routine` context**                                                                          | the document permissions are context-scoped (`HasDynamicPermission`): Admin's existing rows carry no context, so without these the enrolment, encounter and vaccination writes the in-use fixture needs answer `NotAllowedToMutateDocument`                                                                                                                                                                                                                                                                 |
