...
pub fn mock_test_service_item() -> MockData {
    ...
    result.invoices = vec![
        ...
        mock_draft_inbound_with_service_lines(),
...
pub fn mock_draft_inbound_with_service_lines() -> InvoiceRow {
    inline_init(|r: &mut InvoiceRow| {
        r.id = "mock_draft_inbound_with_service_lines".to_string();
        r.name_id = mock_name_a().id;
        r.store_id = "store_a".to_owned();
...
pub fn mock_draft_inbound_service_line() -> InvoiceLineRow {
    inline_init(|r: &mut InvoiceLineRow| {
        ...
        r.invoice_id = mock_draft_inbound_with_service_lines().id;
        r.item_id = mock_default_service_item().id;
    })
}
