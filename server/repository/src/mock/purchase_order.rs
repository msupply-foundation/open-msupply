...
pub fn mock_purchase_order_b_confirmed() -> PurchaseOrderRow {
    inline_init(|r: &mut PurchaseOrderRow| {
        r.id = "test_purchase_order_b_confirmed".to_string();
        r.store_id = mock_store_a().id;
        r.status = db_diesel::purchase_order_row::PurchaseOrderStatus::Confirmed;
        r.supplier_name_link_id = "name_a".to_string();
...
pub fn mock_purchase_orders() -> Vec<PurchaseOrderRow> {
    vec![mock_purchase_order_a(), mock_purchase_order_b_confirmed()]
}
