...
use util::{constants::APPROX_NUMBER_OF_DAYS_IN_A_MONTH_IS_30, inline_edit, inline_init, uuid::uuid};
...
pub fn mock_item_stats() -> MockData {
    ...
    .join(inline_edit(&consumption_points(), |mut u| {
        u.invoices[0].picked_datetime =
            Some(Utc::now().naive_utc() - Duration::days((APPROX_NUMBER_OF_DAYS_IN_A_MONTH_IS_30 + 2.0) as i64));
    ...
    .join(inline_edit(&consumption_points(), |mut u| {
        u.invoices[0].picked_datetime = Some(
            Utc::now().naive_utc() - Duration::days((APPROX_NUMBER_OF_DAYS_IN_A_MONTH_IS_30 * 3.0 + 1.0) as i64),
        );
