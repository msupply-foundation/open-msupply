...
pub(crate) async fn setup_with_version(
    ...
    let root_connection = pool.get().expect("Failed to open connection");
    ...
    let template_settings = get_test_db_settings(&template_name);
    {
        ...
        let existing_templates: Vec<String> = pg_database::table
            ...
            .filter(pg_database::dsl::datname.like("___template_%"))
            .load(&root_connection)
            .unwrap();

        // only clear the DB once, i.e. use the repository test_output directory
        ...
        if !template_dbs.contains(&template_settings.database_name) {
            let connection_manager =
                create_template_db(&root_connection, &template_settings, version.clone());
            let connection = connection_manager.connection().unwrap();
