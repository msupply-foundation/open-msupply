...
pub(crate) fn migrate(connection: &StorageConnection) -> anyhow::Result<()> {
    ...
    .is_err()
    {
        println!("Database migration: Failed to add is_sync_update column to name, name_store_join, clinician and clinician_store_join");
    }

    // these statements would have had the same problem
    if migrate_triggers(connection).is_err() {
        println!("Database migration: Failed to add triggers for is_sync_update");
    }
