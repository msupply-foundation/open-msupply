...
impl MigrationFragment for Migrate {
    fn identifier(&self) -> &'static str {
        "reintegrate_categories_for_custom_field_options"
    }

    /// Backfill `custom_field_option` for legacy item **and** name categories.
    ///
    /// `CategoryTranslation` / `NameCategoryTranslation` now (central-only) also
    /// emit a `custom_field_option` row per category record, mapping mSupply
    /// categories onto the new `item_category*` / `name_category*` OPTION
    /// custom_fields. But the original category records were integrated before
    ...
    /// custom_fields are seeded), authoring the options with no re-initialisation and
