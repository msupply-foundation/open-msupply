...
impl MigrationFragment for Migrate {
    ...
    fn migrate(&self, connection: &StorageConnection) -> anyhow::Result<()> {
        sql!(
            connection,
            r#"
            ALTER TABLE invoice ADD COLUMN properties_v2 {JSONB};
            "#
        )?;
        ...
        if cfg!(feature = "postgres") {
            sql!(
                ...
                UPDATE invoice
                SET properties_v2 = jsonb_build_object(
                ...
                WHERE sb.record_id = invoice.id
                  ...
                  AND invoice.properties_v2 IS NULL
...
mod test {
    ...
    #[actix_rt::test]
    async fn migration_add_invoice_properties_v2() {
        let previous_version = v2_18_00::V2_18_00.version();
        let version = v3_00_00::V3_00_00.version();

        let SetupResult { connection, .. } = setup_test(SetupOption {
            db_name: &format!("migration_{version}_add_invoice_properties_v2"),
        ...
        let value = |id: &str| {
            sql_query("SELECT CAST(properties_v2 AS TEXT) AS properties FROM invoice WHERE id = $1")
                .bind::<Text, _>(id)
                .get_result::<PropertiesRow>(connection.lock().connection())
                .unwrap()
                .properties
