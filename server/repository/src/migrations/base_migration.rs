...
// pg_dump -h localhost -U postgres -d ___template_2.15.0 --inserts --no-owner --no-privileges --schema=public | sed -E '/^\\(un)?restrict [A-Za-z0-9]+$/d' | sed '/^CREATE SCHEMA public;$/d' | sed "s/SELECT pg_catalog.set_config('search_path', '', false);/SET search_path = public;/" | sed '/^SET transaction_timeout = 0;$/d' > repository/src/migrations/base_migrations/postgres_latest.sql
#[cfg(feature = "postgres")]
