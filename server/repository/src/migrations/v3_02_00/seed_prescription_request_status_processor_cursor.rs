...
/// Seed the prescription-request status processor's cursor at the current
/// changelog head. No invoice can reference a prescription_request before this
...
impl MigrationFragment for Migrate {
    fn identifier(&self) -> &'static str {
        "seed_prescription_request_status_processor_cursor"
    }
    ...
    fn migrate(&self, connection: &StorageConnection) -> anyhow::Result<()> {
        sql!(
            ...
            SELECT 'PRESCRIPTION_REQUEST_STATUS_PROCESSOR_CURSOR', COALESCE(MAX(cursor), 0)
            FROM changelog;
            "#
        )?;
