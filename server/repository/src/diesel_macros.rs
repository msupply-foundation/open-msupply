...
macro_rules! diesel_string_enum {
    // Arm: trailing `@catchall` variant for forward-compat with values added
    // by newer central servers. Requires `Other(String)`-style tuple variant
    // marked with `#[strum(default)]` so unknown values round-trip through it.
    // Hand-rolls `as_str` / `AsRef<str>` / `Display` because strum's derives
    // would emit the variant name (`"Other"`) instead of the contained string.
    //
    // Usage:
    // ```
    // diesel_string_enum! {
    ...
    (
        ...
        impl $name {
            fn as_str(&self) -> &str {
                match self {
                    ...
                    Self::$default(value) => value.as_str(),
                }
            }
        }
        ...
        impl From<String> for $name {
            fn from(value: String) -> Self {
                use std::str::FromStr;
                // `from_str` is infallible because of the `#[strum(default)]` variant.
