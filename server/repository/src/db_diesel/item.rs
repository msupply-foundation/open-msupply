use super::{
    ...
    item_warning_join_row::item_warning_join, master_list_line_row::master_list_line,
...
fn create_filtered_query(store_id: String, filter: Option<ItemFilter>) -> BoxedItemQuery {
    ...
    if let Some(f) = filter {
        ...
        if let Some(item_warning_join_id_filter) = item_warning_join_id {
            let mut sub_query = item_link::table
                .select(item_link::item_id)
                .inner_join(
                    item_warning_join::table.on(item_warning_join::item_link_id.eq(item_link::id)),
                )
                .into_boxed();
            apply_equal_filter!(
                sub_query,
                Some(item_warning_join_id_filter),
