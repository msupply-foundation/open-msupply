...
FROM dos_result
  WITH inner_query AS (
    ...
    FROM (
        ...
        FROM stock_movement
        WHERE (
            (stock_movement.item_id IN ('item_a'))
            AND (stock_movement.store_id = 'store_a')
          )
      )
  ),
  variables AS (
    SELECT '2025-12-19T10:59:59.999999999' AS start_datetime,
      '2025-12-30T10:59:59.999999999' AS end_datetime
  ),
  starting_stock AS (
    SELECT item_id,
      store_id,
  ...
  ledger AS (
    SELECT *,
      julianday(date(datetime, 'localtime')) AS date
    FROM starting_stock
    UNION
    SELECT *,
      julianday(date(datetime, 'localtime')) AS date
  ...
  days_with_no_stock AS (
    SELECT item_id,
      ...
      CASE
        WHEN end_of_day_stock <= 0 THEN date + 1
        ELSE date
      END AS date
    FROM daily_stock
  ),
