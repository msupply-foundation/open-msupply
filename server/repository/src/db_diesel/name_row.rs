...
impl<'a> NameRowRepository<'a> {
    ...
    pub fn update_properties(
        ...
        // Only return name changelog if properties contains "supply_level"
        if let Some(props) = properties {
            if props.contains("supply_level") {
                return self.insert_changelog(name_id.to_owned(), RowActionType::Upsert);
            }
        }
