...
use crate::{ItemLinkRowRepository, StorageConnection};
use diesel::prelude::*;
...
impl<'a> RequisitionLineRowRepository<'a> {
    ...
    pub fn upsert_one(&self, row: &RequisitionLineRow) -> Result<(), RepositoryError> {
        // `_upsert` is generated by `define_linked_tables!`; it maps the resolved
        // `item_id` back onto the `item_link_id` write column on the core table.
        self._upsert(row)?;
...
impl Upsert for RequisitionLineRow {
    fn upsert_sync(
        ...
        RequisitionLineRowRepository::new(con)._upsert(self)?;
