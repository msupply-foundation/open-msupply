...
impl<'a> SyncFileReferenceRowRepository<'a> {
    ...
    ///
    /// A row that had given up is revived. `find_all_to_download` keeps such a row out of
    /// the queue by way of `retries < max_retries`, so without this a file that exhausted
    /// its retries could never be wanted again and asking for it a second time would
    /// silently do nothing. Which is why `retries` — not just the status — is what has to
    /// reset; `PermanentFailure` is only how we recognise the row.
    ///
