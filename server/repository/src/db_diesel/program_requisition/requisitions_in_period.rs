...
use crate::{
    ...
    EqualFilter, RequisitionType, StorageConnection, StringFilter,
};
...
mod test {
    ...
    async fn requisitions_in_period_repository() {
        ...
        let mut filter = RequisitionsInPeriodFilter::new()
            .program_id(EqualFilter::equal_to_string(&program1.id.clone()))
        ...
        let mut result = repo
            .query(
                RequisitionsInPeriodFilter::new()
                    .store_id(EqualFilter::equal_to_string(&mock_store_b().id)),
            )
            .unwrap();
        result.sort_by(sort);
