...
Options:
  ...
  -n, --n-values <VALUES>    Run only these N values (comma-separated)
      ...
      --generate-graphs <PATH>  Regenerate charts from existing results.json
  -h, --help                 Print help
```
...
# Run specific table sizes only
cargo run -- --n-values 1200000,3400000

# Skip chart generation
cargo run -- --no-graphs

# Regenerate seed templates (if changelog schema changes)
...
A full run can take a long time (the tested table sizes of 1M to 100M rows took ~18 hours). On macOS, use `caffeinate` to prevent the system from sleeping:
...
Everything is driven by `config.toml`. The `[pg]` section and `[null_profiles]` are optional.
...
| `scenarios[].partition_file` | (optional)         | Path to partition .sql file                                               |
...
3. Run the benchmarks. Seed databases are generated automatically on first run for each table size. A full run can take a long time (the tested table sizes of 1M to 100M rows took ~18 hours), so it is recommended to run the process in the background rather than directly in the terminal. This keeps the benchmarks running if the terminal is closed or the session is disconnected.
   ...
   A new, empty terminal window will open. Leave this open during the process.
