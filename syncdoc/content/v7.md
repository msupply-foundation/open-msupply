...
Changelog table will grow over time to be very big, it needs to be fast when queried, since it's integral part of sync we want to reduce overhead of that query. Although various optimisations can be done to indexing and queries, the best solution of the problem is sometimes the absence of the problem. We shouldn't try to de-duplicate on the go, only do it as a scheduled task, allow for duplicates and deal with those in application logic. This would mean
...
4. Together with inert only nature of sync_buffer we would have multiple operations in sync_buffer for the same record, in order to deal with this, we would de-duplicate in application logic (TODO link to sync buffer about this, to be added when sync_buffer research issues has PR)
