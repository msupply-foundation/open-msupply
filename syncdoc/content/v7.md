...
Tables that have more than one sync style (for example `Invoice`, which is Remote + Transfer + Patient; `Encounter`, which is Central + Patient) are pulled if any of the applicable conditions match. This is the OR in the Example Pull query.
...
| Remote | Accept when `store_id` is present. Source site identity is verified by token authentication, and `source_site_id` is stamped at mutation time. | Accept when `store_id` is one of the active stores on this site, unless the row's `source_site_id` is this site itself and the site is not initialising — own echoes are rejected, but rows authored elsewhere (central-authored store-scoped data, or incoming store-moves) are accepted. |
...
| RemoteToCentral | Accept when `store_id` is present (same as Remote). | Reject — these records are deliberately one-way and are never sent back to a remote, including on re-initialisation. |
