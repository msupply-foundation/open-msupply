...
describe('getQuantity', () => {
  describe('when itemType is units', () => {
    it('should return pack quantity with precision 2', () => {
      expect(getQuantity('units', 10, 4)).toBe(2.5);
    });

    it('should handle decimal values correctly', () => {
      expect(getQuantity('units', 10.5, 3)).toBe(3.5);
    });

    it('should return 0 when requestedQuantity is undefined', () => {
      expect(getQuantity('units', undefined, 2)).toBe(0);
    });

    it('should use 1 as default packSize when undefined', () => {
      expect(getQuantity('units', 10, undefined)).toBe(10);
    });
  });

  describe('when itemType is packs', () => {
    it('should return ceiling of requestedQuantity', () => {
      expect(getQuantity('packs', 10, 2)).toBe(10);
    });

    it('should round up decimal values', () => {
      expect(getQuantity('packs', 10.1, 2)).toBe(11);
    });

    it('should return 0 when requestedQuantity is undefined', () => {
      expect(getQuantity('packs', undefined, 2)).toBe(0);
    });
  });
});
