import { ContraceptiveIcon } from './Contraceptive';
import { DocsIcon } from './Docs';
import { GenderBasedViolenceIcon } from './GenderBasedViolence';
import { GynaecologicalIcon } from './Gynaecological';
import { HaemotologyIcon } from './Haemotology';
import { HeartIcon } from './Heart';
import { ImmunizationIcon } from './Immunization';
import { IndexTestingIcon } from './IndexTesting';
import { LiverIcon } from './Liver';
import { LungsIcon } from './Lungs';
import { LungsVirusIcon } from './LungsVirus';
import { MedicalBagIcon } from './MedicalBag';
import { MedicalClipboardIcon } from './MedicalClipboard';
import { MedicalHistoryIcon } from './MedicalHistory';
import { MedicationIcon } from './Medication';
import { MicroscopeIcon } from './Microscope';
import { NutritionIcon } from './Nutrition';
import { PregnancyIcon } from './Pregnancy';
import { ReferralsIcon } from './Referrals';
import { StethoscopeIcon } from './Stethoscope';
import { TestTubeIcon } from './TestTube';
import { VirusIcon } from './Virus';
import { VisitTypeIcon } from './VisitType';

// Use this object when you need to refer to an icon dynamically, e.g. when
// defined in JSON Forms schemas
export const MedicalIcons = {
  pregnancy: PregnancyIcon,
  maternalHistory: PregnancyIcon,
  heart: HeartIcon,
  referrals: ReferralsIcon,
  stethoscope: StethoscopeIcon,
  testTube: TestTubeIcon,
  microscope: MicroscopeIcon,
  contraceptive: ContraceptiveIcon,
  gbv: GenderBasedViolenceIcon,
  gynaecological: GynaecologicalIcon,
  haemotology: HaemotologyIcon,
  immunization: ImmunizationIcon,
  injection: ImmunizationIcon,
  indexTesting: IndexTestingIcon,
  liver: LiverIcon,
  lungsVirus: LungsVirusIcon,
  lungs: LungsIcon,
  medicalBag: MedicalBagIcon,
  medicalHistory: MedicalHistoryIcon,
  medication: MedicationIcon,
  medicine: MedicationIcon,
  nutrition: NutritionIcon,
  medicalClipboard: MedicalClipboardIcon,
  riskBehaviour: MedicalClipboardIcon,
  documents: DocsIcon,
  riskBehaviourAlt: DocsIcon,
  virus: VirusIcon,
  viralLoad: VirusIcon,
  visitType: VisitTypeIcon,
  visit: VisitTypeIcon,
};

export {
  ContraceptiveIcon,
  GenderBasedViolenceIcon,
  PregnancyIcon,
  HeartIcon,
  ReferralsIcon,
  StethoscopeIcon,
  TestTubeIcon,
  MicroscopeIcon,
  GynaecologicalIcon,
  HaemotologyIcon,
  ImmunizationIcon,
  IndexTestingIcon,
  LiverIcon,
  LungsVirusIcon,
  LungsIcon,
  MedicalBagIcon,
  MedicalHistoryIcon,
  MedicationIcon,
  NutritionIcon,
  MedicalClipboardIcon,
  DocsIcon,
  VirusIcon,
  VisitTypeIcon,
};
