...
import { DraftProperties, useDraftProperties } from '@common/utils';
import {
  PropertyV2DetailRows,
  PropertyV2RenderDefinition,
} from './PropertyV2DetailRows';

interface PropertiesEditTabProps {
  /** Property definitions for the record kind, in the order to display them. */
  definitions: PropertyV2RenderDefinition[];
  /** The record's loaded `propertiesV2` blob; the draft initialises from it. */
 ...
 * propertiesV2 tab (patient, invoice, …): owns the draft, renders the labelled
 * rows via {@link PropertyV2DetailRows}, and saves behind a confirmed Save
 ...
 */
export const PropertiesEditTab = ({
  definitions,
  properties,
  onSave,
  onEdit,
  disabled,
  isLoading,
  saveErrorMessage = 'error.failed-to-save-properties',
}: PropertiesEditTabProps) => {
  const t = useTranslation();
  const { error, success } = useNotification();
  const [isSaving, setIsSaving] = useState(false);

  const { draftProperties, updateProperty, isDirty } =
    useDraftProperties(properties);
