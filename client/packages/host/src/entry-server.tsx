/**
 * SSR entry point used only at build time by prerenderPlugin in vite.config.ts.
 *
 * Renders a self-contained login-page shell — same layout and colours as the
 * real Login component, but with zero browser-dependent dependencies
 * (localStorage, auth context, i18n, Emotion, MUI).  All styles are inlined so
 * no CSS-extraction step is required.
 *
 * The resulting HTML is injected into dist/index.html by scripts/prerender.mjs
 * so the browser gets a fully-painted login page before any JavaScript runs,
 * improving FCP / LCP.
 *
 * React hydrates this shell with the real App transparently (see bootstrap.tsx
 * which calls hydrateRoot when #root already contains children).
 */

import React from 'react';
import { renderToString } from 'react-dom/server';

/* ── Design tokens (kept in sync with packages/common/src/styles/theme.ts) ── */
const TOKEN = {
  gradientLeft: 'linear-gradient(156deg, #f80 4%, #e63535 96%)',
  bgLogin: '#f2f2f5',
  textWhite: '#fafafa',
  textDark: '#1c1c28',
  textMuted: '#8f90a6',
...
/* ── mSupplyGuy SVG (paths from MSupplyGuy.tsx) ───────────────────────────── */
const MSupplyGuyIcon = ({ width = 285, height = 180 }: { width?: number; height?: number }) => (
  <svg
    viewBox="3 1 206 310"
  ...
  >
    <g>
      <linearGradient id="ssr-lg" x1="0.5" y1="0" x2="0.5" y2="1">
      ...
      />
      <path fill="#fff" d="M83.1612,97.3916 L107.319,104.638 L107.016,106.148 L81.35,101.922 Z" />
      <path
        fill="#fffffe"
...
const LoginShell = () => (
  <>
    {/* Critical CSS: only what inline styles cannot express (media query, font). */}
    <style
      dangerouslySetInnerHTML={{
        __html: [
          `*{box-sizing:border-box}`,
          // Hide the gradient panel on narrow viewports (mirrors the LoginLayout sx)
          `@media(max-width:600px){.ssr-left{display:none!important}}`,
        ].join(''),
      }}
    />
    ...
    <div style={{ display: 'flex', width: '100%', minHeight: '100vh' }}>
      ...
      <div
        ...
        style={{
          flex: '1 0 50%',
          display: 'flex',
          backgroundImage: TOKEN.gradientLeft,
          padding: '0 5% 7%',
        ...
        >
          <form style={{ display: 'flex', flexDirection: 'column', gap: 40 }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'center' }}>
              <MSupplyGuyIcon width={285} height={180} />
            </div>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
              <button
                ...
                style={{
                  fontFamily: TOKEN.font,
                  border: `1px solid ${TOKEN.primaryMain}`,
                  borderRadius: 4,
                  backgroundColor: 'transparent',
                  color: TOKEN.primaryMain,
                  padding: '5px 15px',
                  fontSize: 14,
                  fontWeight: 500,
                  cursor: 'default',
...
/**
 * Renders the login shell to a plain HTML string.
 * All styles are inline so no separate CSS extraction is needed.
 * Called by scripts/prerender.mjs after this SSR bundle is built.
 */
export function render(): { html: string; css: string } {
  const html = renderToString(<LoginShell />);
  // No separate style tags to inject — everything is inlined above.
  return { html, css: '' };
}
