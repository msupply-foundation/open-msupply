...
export const useInboundShipment = (id?: string) => {
    ...
    const create = async (input: Omit<InsertInboundShipmentMutationVariables, 'storeId'>) => {
...
const useCreate = () => {
    ...
    const mutationFn = async (
        ...
        throw new Error(insertInboundShipment?.error?.description || 'Could not create invoice');
    };
