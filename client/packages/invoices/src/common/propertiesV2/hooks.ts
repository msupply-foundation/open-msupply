...
/**
 * Fetch the propertiesV2 definitions for one invoice type's scope. The value
 ...
 * OPTION values.
 */
export const useInvoicePropertiesV2 = (invoiceType: InvoiceNodeType) => {
  const { api } = usePropertiesV2GraphQL();
  ...
  return useQuery({
    ...
    queryFn: async () => {
      if (!tableName) return [];
      const result = await api.invoicePropertiesV2({ tableName });
