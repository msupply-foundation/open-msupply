...
/**
 * Renders the invoice type's `PROMINENT` propertiesV2 (e.g. the legacy
 ...
 */
export const InvoiceToolbarProperties = ({
  invoiceType,
  propertiesV2,
  onUpdate,
  disabled,
}: InvoiceToolbarPropertiesProps) => {
  const { userHasPermission } = useAuthContext();
  const { data: definitions = [] } = useInvoicePropertiesV2(invoiceType);

  const prominent = definitions.filter(
    definition => definition.displayMode === PropertyNodeDisplayModeV2.Prominent
  );
  ...
  const values = (propertiesV2 ?? {}) as Record<string, unknown>;
