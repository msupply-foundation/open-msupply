...
@REM cd cargo build --release && copy "target\release\remote_server.exe" "..\omSupply\Server\omSupply-server-sqlite.exe"
@REM @if %errorlevel% neq 0 exit /b %errorlevel%
...
copy c:\android\local.properties packages\android\ /y
copy c:\android\release.keystore packages\android\app\ /y
