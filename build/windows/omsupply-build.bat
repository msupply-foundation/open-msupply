...
@if %errorlevel% neq 0 (set ERROR_STEP=Client Preparation & goto :error)
...
@if %errorlevel% neq 0 (set ERROR_STEP=SQLite Build & goto :error)
...
@if %errorlevel% neq 0 (set ERROR_STEP=SQLite Server Build & goto :error)
...
@if %errorlevel% neq 0 (set ERROR_STEP=Postgres Build & goto :error)
...
@if %errorlevel% neq 0 (set ERROR_STEP=CLI Build & goto :error)
...
@if %errorlevel% neq 0 (set ERROR_STEP=Connection Test Build & goto :error)
...
@if %errorlevel% neq 0 (set ERROR_STEP=Electron Build & goto :error)

@REM Success - pass "success" as argument
python .github\scripts\send-telegram-notification.py success
goto :end

:error
@REM Failure - pass "failure" as argument  
python .github\scripts\send-telegram-notification.py failure "%ERROR_STEP%"
exit /b %errorlevel%
