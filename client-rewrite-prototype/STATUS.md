...
Begin **decision #3 — UI library & styling approach**. Decisions #1 (**React**) and #2 (**Vite**) are ratified — see `DECISIONS.md`. The Vite/React 19/TS skeleton is scaffolded and verified (builds, lints, dev-serves on :3003) at [`app/`](./app/) — see "Done".
...
- App scaffold setup choices (`app/`): **yarn 4** via corepack as a *standalone* project (empty `yarn.lock` marks it separate from the repo root, which has no workspaces), `node-modules` linker + 7-day supply-chain age gate (mirrors `client/.yarnrc.yml`); **TypeScript pinned to ^5.9** (matches the repo, not the brand-new TS 6 — also keeps typescript-eslint happy); dev port **:3003** (reuses the existing host port); **`@/` → `src/`** import alias. Vite config is intentionally minimal + Module-Federation-compatible for the deferred plugin work.
...
3. **UI library & styling approach** — headless components for hard widgets + a styling method (CSS Modules vs vanilla-extract vs Tailwind vs other). NOT decided. _(Spec: runtime CSS-in-JS is now ruled out; theming must support the full TMF token set — see `SPEC.md`.)_
