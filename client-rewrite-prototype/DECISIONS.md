...
## 2026-07-01 · Theming — attribute-flip over CSS custom properties (+ a dark theme)

- **Decision:** Themes are switched by setting one attribute, **`data-theme`**, on `<html>`; each theme is a **declarative CSS override block** (`:root[data-theme='dark'] { … }`) that only re-values the existing design tokens in `styles/tokens.css`. Components never learn a theme exists — they read tokens, and the cascade recolours everything. JS owns exactly one thing: the attribute. This is the direct analogue of how `LocaleProvider` flips `dir`/`lang` (see the RTL cross-cutting note), so theming and direction share one mental model.
- **Mechanism built:** `theme/ThemeProvider` (mirrors `LocaleProvider`) holds the state, writes `data-theme`, and persists the choice to `localStorage`; `theme/themeContext` is the split context+hook (Fast-Refresh-clean, like `localeContext`); a footer **`ThemeToggle`** (sun/moon) flips it. Default is **light for everyone** — `prefers-color-scheme` is deliberately **not** consulted (existing users' experience is unchanged unless they opt in). A ~5-line **pre-paint inline script in `index.html`** applies the stored theme before React mounts, so a dark user never sees a flash of light. Verified in-browser: toggle flips all surfaces/text/borders, and the choice survives reload with no flash.
- **Why this over JS `setProperty`:** setting vars imperatively from JS scatters the palette across runtime calls and makes "what is the current theme" an imperative question. An override block keeps every theme's palette in one reviewable place next to the light defaults, diffable in git, with zero per-property JS. `setProperty` stays reserved for the genuinely *dynamic* case below.
- **Palette approach (dark):** navy-tinted (matches the palette's existing navy greys, not neutral black); an **inverted surface scale** — elevated surfaces get *lighter*, so content sits above chrome exactly as in light mode, mirrored; the **`-dark` emphasis variants flip to lighter shades** since brighter = more emphasis on a dark ground; brand orange kept unchanged for identity; text is soft near-white, not pure white; `color-scheme: dark` set so native scrollbars/controls/autofill follow. Only token *values* change — no component or structural CSS was touched.
- **Elevation model — the one place a *new* token was needed.** Light mode conflated "page/content surface" and "raised surface" (both `--bg-white` = `#fff`, separated only by shadow). That breaks on dark: a raised surface the same colour as content is invisible, and the light theme's *grey* shadows are lighter than a dark surface so they read as a **glow**, not depth. Fix: a semantic **`--surface-raised`** token (= `--bg-white` in light, unchanged; **lighter** than content, `#2b2b3a` vs `#21212b`, in dark) applied to every floating surface (buttons, split button, menus, dialog, Select/Combobox/MultiSelect popups, language menu, perf HUD); **plus** the dark `--shadow-*` tokens overridden to **near-black**. On dark the two work together — the lighter surface separates, the black shadow reinforces — mirroring how Material's dark theme lightens surfaces with elevation. Sidebar/drawer deliberately excluded (they *recede*, staying darker than content). Verified in-browser: buttons and popups now read as raised.
- **Inputs stay flat + bordered (deliberate), and their dark border was fixed.** All four input types (`TextField` + the Select/Combobox/MultiSelect triggers) consistently use `--bg-white` for the enabled fill and rely on a border for their edge — `--bg-input` is only the *disabled* fill. So on dark the field fill matches content (`#21212b`) and the **border** defines the field. The initial dark block overrode every border token *except* `--input-border`, leaving inputs with a near-white `#e4e4e7` edge (harsh on dark); fixed by overriding `--input-border` to `#3d3e4c` in the dark block. (An earlier note here wrongly claimed the triggers diverged from `TextField` — they don't; the real gap was the missing border override.)
- **Directly serves the spec's TMF-theming criterion.** The same block mechanism is how per-org brand themes would ship. **Not yet built** but the intended extension: split *primitive* seed tokens from *semantic* tokens so a tenant overrides a handful of seeds (e.g. one brand colour) and `color-mix()` derives the rest — and *that* runtime-unknown case is the one place JS `setProperty` on `:root` is the right tool. Recorded now so the split is done deliberately while the token set is small.
- **Alternatives rejected:** JS `element.style.setProperty` per token as the primary mechanism (imperative, scattered, no single source of truth — kept only for future runtime-supplied themes); a separate dark stylesheet (duplicates structure, drifts); `prefers-color-scheme`-only with no toggle (no user override, and changes existing users' default unasked).
- **Status:** Adopted; light + dark built and verified.
