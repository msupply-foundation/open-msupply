 ...
 *   - Focus management: Tab moves from the selected tab straight into its panel.
 *   - Inactive panels are unmounted from the DOM (cheap; no hidden work).
 *
 * The primitive is tiny — no positioning / Floating UI — so the cost is minimal;
 * we're buying the a11y contract, not a look. The look is entirely our CSS,
 * including the sliding underline (below).
 */
export const Tabs = RadixTabs.Root;

/*
 * TabList renders a single underline indicator that SLIDES between labels. Radix
 * doesn't ship an animated indicator, so we measure the active tab's geometry
 * (the one legit px use — computed layout, not a design token) and translate the
 * bar. Re-measures on tab change and on resize (which also covers the root-size
 * change at the phone breakpoint).
 */
