...
/*
 * The single switch. `impl` selects the provider; that is the whole mechanism the
 * benchmark toggles. Mount it with a key of `${impl}:${fields}` so changing either
 * gives a clean remount and no stale store/state leaks between conditions.
 */
export const StateProvider = ({
  ...
  if (impl === 'context-memo') {
    return (
      <ContextMemoStateProvider initial={initial}>
        {children}
      </ContextMemoStateProvider>
    );
  }
