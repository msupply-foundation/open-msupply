...
To disable auto-spawn (e.g. for backend-only work or to run webpack manually), add `dev_server: false` under the `server:` key in `configuration/local.yaml`. The server will serve the embedded frontend bundle instead (run `yarn build` in `client/` first).
