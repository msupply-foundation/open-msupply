...
test.describe('Outbound Shipments — report-specific filters', () => {
  test.beforeEach(async ({ reportsListPage }) => {
    await reportsListPage.goto();
    await reportsListPage.expectPageLoaded();
    await reportsListPage.clickReport('Outbound Shipments');
  });
  ...
  test('displays expected filter fields', async ({
    ...
    await reportArgumentsModal.expectFieldVisible('Customer');
