...
- **Scope.** `custom_1/2/3` are seeded for both `name` and `patient` (`central_mapping_custom_fields`); `PatientNode.customFields` filters to the `patient` scope, so patients can surface a different visible set from suppliers/facilities (which use `name`).
