...
- **Authoring** — _who is allowed to create or edit this record and push it?_ This gates what a receiver accepts on an incoming push — checked both when central receives a push from a remote and when a remote receives one back.
- **Distribution** — _which sites are eligible to receive this record on pull-down?_ This gates which sites central sends a row to.
...
Because authoring is a set, a push is accepted if _any_ clause accepts it. A patient-typed name, for example, is tagged `Central + Patient`: central rejects it under the `Central` clause but accepts it under the `Patient` clause, so the only name-push central accepts is a patient one.
...
| **Transfer**       | The site that owns the _other_ store referenced by the record.                                                                         |
...
| `NameOmsFields`               | Anyone                           | Central                          | v6        |
...
- **Remote + Patient · Remote + Patient · v6** (`Encounter`, `Vaccination`, `ContactTrace`) — store-scoped clinical records that should also follow the patient. Each row carries the authoring store _and_ the patient. The `Remote` clause delivers it to the owning site; the `Patient` clause delivers it to every other site that knows the patient. Authoring is `Remote` (not `RemoteOwned`): the owning site keeps accepting updates even after it has initialised.
...
| `NameOmsFields`                                                                   | A name's properties, broadcast as their own record. Its translator opts in to the OMS-central pull so v6 sites receive it. Authoring is `Anyone`, so central accepts a remote → central writeback (a store editing its own name's properties) — but that writeback rides v7: the OMS-push translation is a no-op here, because the table isn't in the bare-row set the push path can surface.                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
The matrix answers _who is eligible and in which direction_. The transport answers _which wire format_. A row only moves when both agree.
