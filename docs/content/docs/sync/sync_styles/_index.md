...
| **v7-only** | Tagged for neither v5 nor v6 — no legacy filter ever selects the table, so it travels only over v7. |
| (no v7 tag) | v7 covers every table that has any sync style. |

Filters that ask for "v6 only" pull just the OMS-native tables. Filters that ask for "v5 only" pull just the legacy-only tables. Filters that pass no transport flag (used by v7) pull every table that has any sync style. A **v7-only** table is therefore invisible to v5 and v6 sites — it starts syncing only once a site is on v7.
...
| `PrescriptionRequest` | RemoteOwned | RemoteOwned | v7-only | — |
| `PrescriptionRequestLine` | RemoteOwned | RemoteOwned | v7-only | — |
...
- **RemoteOwned · RemoteOwned · v7-only** (`StockRelocation`, `StockRelocationLine`, `PrescriptionRequest`, `PrescriptionRequestLine`) — site-owned OMS-native records with no legacy counterpart at all: no v5 or v6 filter ever selects them, so they move only between v7 sites and OMS central. Routing behaves like the other RemoteOwned cells — the owning site pushes, and the return path is suppressed except at initialisation.
...
- **Central · Central · v7-only** (`CustomField`, `CustomFieldOption`, `CustomFieldScope`) — the properties-v2 definitions, authored on OMS central and broadcast, but only over v7. They are multi-device-tagged because multi-device sites sync names (which carry custom-field *values*), so the definitions must reach those devices too.
...
| Store only | `StockLine`, `StockRelocation`, `PrescriptionRequest`, `Stocktake`, `Location`, `PurchaseOrder`, `Preference`, `Sensor`, `TemperatureBreach`, `TemperatureLog`, `VVMStatusLog`, `LocationMovement`, `ActivityLog`, `ContactForm`, `Asset`, `PluginData`, `VaccineCourseStoreConfig`, `ClinicianStoreJoin`, `IndicatorValue`, `ItemStoreJoin`, `UserPermission`, `UserStoreJoin` | Just the row's own store. |
...
| Line inherits parent | `InvoiceLine` ← `Invoice`, `StocktakeLine` ← `Stocktake`, `RequisitionLine` ← `Requisition`, `RnrFormLine` ← `RnrForm`, `StockRelocationLine` ← `StockRelocation`, `PrescriptionRequestLine` ← `PrescriptionRequest` | The line's changelog is built from the parent's, then `table_name` and `record_id` are overridden to point at the line. Guarantees parent and line stay aligned for store / transfer-store / patient / source-site, so they route together. |
