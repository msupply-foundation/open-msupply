...
- The synthesized join is an ordinary `name_store_join` (fresh id) — like any other central-authored join it is pushed to OG. Records integrate one at a time, so a later prescription for the same patient & store sees the join and doesn't synthesize another.
